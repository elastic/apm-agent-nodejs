...
function Queue (opts, onFlush) {
  ...
  this._items = []
  this._timeout = null
  this._flushInterval = opts.flushInterval * 1000
...
Queue.prototype.add = function (obj) {
  ...
  if (this._items.length >= this._maxQueueSize) this.flush()
  else if (!this._timeout) this._queueFlush()
}

Queue.prototype.flush = function (cb) {
  this.logger.debug('flushing queue')
...
Queue.prototype._queueFlush = function () {
  ...
  this.logger.debug('setting timer to flush queue: %dms', ms)
