...
var Client = function Client(options) {
    ...
    this.uncaught     = 'uncaughtExceptions' in options ? options.uncaughtExceptions : true;
    this.dsn          = {
...
_.getIdent = function getIdent(result) {
    return result.id;
};
...
_.process = function process(kwargs, cb) {
    ...
    if (cb) {
        this.once('error', cb);
        this.once('logged', function (result) {
            cb(null, result);
        });
    }
...
_.captureError = function captureError(err, kwargs, cb) {
    var client = this;
...
_.captureRequestError = function captureRequestError(err, req, kwargs, cb) {
    if (!cb && typeof kwargs === 'function') {
        cb = kwargs;
        kwargs = {};
    } else {
...
_.handleUncaughtExceptions = function handleUncaughtExceptions(callback) {
    var client = this;

    callback = callback || function (opbeatErr, result) {
        if (opbeatErr) {
            node_util.log('Could not send error to Opbeat:');
            node_util.log(opbeatErr.stack);
        } else {
            node_util.log('Logged to Opbeat with id: ' + client.getIdent(result));
        }
        process.exit(1);
    };

    process.on('uncaughtException', function (err) {
        node_util.log('Opbeat caught unhandled exception:');
