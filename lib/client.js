...
var Client = function (options) {
    ...
    if (['development', 'test'].indexOf(this.env) !== -1) {
        if (!this.silent) console.warn('Warning: Opbeat logging is disabled while running in %s mode', this.env);
        this._enabled = false;
    } else if (!this.organization_id || !this.app_id || !this.secret_token) {
        if (!this.silent) console.warn('Warning: Opbeat logging is disabled. To enable, specify organization id, app id and opbeat token');
        this._enabled = false;
    } else {
        this._enabled = true;
    }
...
_.handleUncaughtExceptions = function (callback) {
    ...
    process.on('uncaughtException', function (err) {
        // TODO: Consider prefix all data logged to STDERR with program name. For more info read:
        ...
        client.captureError(err, function (opbeatErr, url) {
          if (!client.silent) {
              if (opbeatErr) {
                  console.warn('Could not send error to Opbeat:');
                  console.warn(opbeatErr.stack);
              } else {
                  console.warn('Logged to Opbeat: ' + url);
              }
          }
