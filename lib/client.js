...
var Client = function (options) {
  ...
  if (['development', 'test'].indexOf(this.env) !== -1) {
    console.info('[WARNING] Opbeat logging is disabled while running in %s mode', this.env);
    this._enabled = false;
  } else if (!this.app_id || !this.organization_id || !this.secret_token) {
    console.info('[WARNING] Opbeat logging is disabled. To enable, specify organization id, app id and opbeat token');
    this._enabled = false;
  } else {
    this._enabled = true;
  }
  ...
  if (this._enabled) {
    ...
    this.on('connectionError', function (err) {
      log.error('Could not notify Opbeat!');
      log.error(err.stack);
    });
...
Client.prototype.handleUncaughtExceptions = function (callback) {
  process.on('uncaughtException', function (err) {
    log.info('opbeat: caught unhandled exception');
