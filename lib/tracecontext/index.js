...
class TraceContext {
  constructor (traceparent, tracestate) {
    ...
    this._hasPropagatedTraceContextHeaders = false
  }
  ...
  propagateTraceContextHeaders (currentHeaders, contextFrom) {
    this._hasPropagatedTraceContextHeaders = true
    return propagateTraceContextHeaders(currentHeaders, contextFrom)
  }

  hasPropagatedTraceContextHeaders () {
    return this._hasPropagatedTraceContextHeaders
  }
}

function propagateTraceContextHeaders (currentHeaders, contextFrom) {
  const headerValue = contextFrom.toTraceParentString()
  const traceStateValue = contextFrom.toTraceStateString()

  const newHeaders = Object.assign({}, currentHeaders)
