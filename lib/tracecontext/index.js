...
class TraceContext {
  ...
  propagateTraceContextHeaders (carrier, contextFrom, setter, useElasticTraceparentHeader) {
    return propagateTraceContextHeaders(
      carrier,
      contextFrom,
      setter,
      this._conf.useElasticTraceparentHeader
    )
  }
}

function propagateTraceContextHeaders (carrier, contextFrom, setter, useElasticTraceparentHeader) {
  if (!carrier || !contextFrom || !setter) {
    return
  }
  if (contextFrom.toTraceParentString()) {
    setter(carrier, 'traceparent', contextFrom.toTraceParentString())
    if (useElasticTraceparentHeader) {
      setter(carrier, 'elastic-apm-traceparent', contextFrom.toTraceParentString())
    }
  }

  if (contextFrom.toTraceStateString()) {
    setter(carrier, 'tracestate', contextFrom.toTraceStateString())
  }
}

TraceContext.FLAGS = TraceParent.FLAGS

module.exports = { TraceContext, propagateTraceContextHeaders }
