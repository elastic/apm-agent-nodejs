...
class TraceContext {
  ...
  // XXX internal _context usage in node-traceparent is a blocker?!?
  // Note: `childOf` can be a TraceContext, a TraceParent, or a thing with a
  // `._traceContext` that is a TraceContext (e.g. GenericSpan).
  static startOrResume (childOf, conf, tracestateString) {
    if (childOf && childOf._traceContext instanceof TraceContext) return childOf._traceContext.child()
