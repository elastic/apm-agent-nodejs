...
// (Internally the Instrumentation has a `this._runCtxMgr` that manipulates
// RunContexts.)
//
// User code is not exposed to RunContexts. The Agent API methods hide those
// details.
//
// A RunContext holds:
// - a current Transaction, which can be null; and
// - a *stack* of Spans, where the top-of-stack span is the "current" one.
//   A stack is necessary to support the semantics of multiple started
//   (`apm.startSpan()`) and ended spans in the same async task. E.g.:
...
// done so that a change in current run context does not change anything for
// other code bound to the original RunContext (e.g. via `ins.bindFunction` or
// `ins.bindEmitter`).
//
// RunContext is roughly equivalent to OTel's `Context` interface.
