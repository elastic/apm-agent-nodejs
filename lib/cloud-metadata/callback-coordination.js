  ...
  constructor (maxWaitMS = -1) {
    ...
    if (maxWaitMS !== -1) {
      this.timeout = setTimeout(() => {
        if (!this.done) {
          this.complete()
          getLogger().error('metadata requests did not finish, possible deadlock')
          const error = new CallbackCoordinationError('callback coordination reached timeout')
  ...
  recordResult (error, result) {
    ...
    if (error) {
      ...
      if (this.resultCount >= this.expectedResults && !this.done) {
        ...
        const error = new CallbackCoordinationError('all callbacks failed')
        error.allErrors = this.errors
        getLogger().debug('no cloud metadata servers responded')
        this.emit('error', error)
      }
    }
