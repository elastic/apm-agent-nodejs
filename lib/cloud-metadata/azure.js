 ...
 */
function getMetadataAzure (host, port, timeoutMs, protocol, cb) {
  const options = {
    method: 'GET',
    timeout: 5000,
  ...
  req.on('timeout', function (error) {
    ...
    if (!requestHasErrored) {
      const cbError = error || new Error('request to azure metadata server timed out')
  ...
  req.on('connectTimeout', function (error) {
    ...
    if (!requestHasErrored) {
      requestHasErrored = true
      const cbError = error || new Error('could not ping azure metadata server')
      cb(cbError)
    }
  })
