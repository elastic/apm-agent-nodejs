...
function formatMetadataStringIntoObject (string) {
  ...
  const metadata = {
    account: {
      id: new String(data.accountId).toString()
    },
    instance: {
      id: new String(data.instanceId).toString()
    },
    availability_zone: new String(data.availabilityZone).toString(),
    machine: {
      type: new String(data.instanceType).toString()
    },
    provider: 'aws',
    region: new String(data.region).toString()
  }
