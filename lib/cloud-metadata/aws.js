 ...
 */
function getMetadataAws (host, port, headers, connectTimeoutMs, resTimeoutMs, protocol, logger, cb) {
  const options = {
    method: 'GET',
 ...
 */
function getMetadataAwsV3 (host, port, connectTimeoutMs, resTimeoutMs, protocol, logger, cb) {
  ...
  const req = httpRequest(
    ...
    function (res) {
      ...
      res.on('end', function () {
        const token = finalData.join('')
        const headers = {}
        if (res.statusCode === 200 && token) {
          headers['X-aws-ec2-metadata-token'] = finalData.join('')
        }
        // the httpRequest function can throw, so we need to catch up here
        // see aws metadata: Unified aws IMDS approach errors for non-aws server
        try {
          getMetadataAws(
            host,
            port,
...
module.exports = { getMetadataAwsV3 }
