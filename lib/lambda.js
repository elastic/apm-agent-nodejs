...
function elasticApmAwsLambda (agent) {
  ...
  function endAndFlushTransaction (err, result, trans, event, context, triggerType, inferredTrans, cb) {
    ...
    if (inferredTrans) {
      // XXX There is so much more metadata we want to override. Almost *all* of it.
      //     However, transaction intake fields do cover all of them.
      inferredTrans.setServiceContext({
        agent: { name: 'some-agent' }, // ???
        name: 'AWS API Gateway', // XXX spaces in service.name is dodgy
        // XXX can we pass `null` for these? or is that then ignored?
        framework: { name: 'XXXframework' },
        language: { name: 'some-language' }, // ???
        runtime: { name: 'managed service' } // ???
      })
      inferredTrans.setOutcome(trans.outcome)
      inferredTrans.end(trans.result)
      // XXX Or do we set the inferred API Gateway span to end at the same time as
      //     trans.end() above? Note that for a coldstart, pre-JITing I've seen
      //     the time from `trans.end()` above to this line take on the order
      //     of 120ms! Consistently.
      //  inferredTrans.end(trans.result, trans.timestamp/1000 + trans._duration)
    }
  ...
  function wrapContext (trans, event, context, triggerType, inferredTrans) {
    shimmer.wrap(context, 'succeed', (origSucceed) => {
      return function wrappedSucceed (result) {
        endAndFlushTransaction(null, result, trans, event, context, triggerType, inferredTrans, function () {
          origSucceed(result)
        })
      }
    })
    ...
    shimmer.wrap(context, 'fail', (origFail) => {
      return function wrappedFail (err) {
        endAndFlushTransaction(err, null, trans, event, context, triggerType, inferredTrans, function () {
          origFail(err)
        })
      }
    })
    ...
    shimmer.wrap(context, 'done', (origDone) => {
      return wrapLambdaCallback(trans, event, context, triggerType, inferredTrans, origDone)
    })
  }

  function wrapLambdaCallback (trans, event, context, triggerType, inferredTrans, callback) {
    return function wrappedLambdaCallback (err, result) {
      endAndFlushTransaction(err, result, trans, event, context, triggerType, inferredTrans, () => {
        callback(err, result)
      })
    }
  }
    ...
    return function wrappedLambda (event, context, callback) {
      ...
      if ((triggerType === TRIGGER_API_GATEWAY || triggerType === TRIGGER_ELB) && event.headers) {
        ...
        if (!event.requestContext.http) { // 1.0
          // XXX These show up in `event` for the *user* code. We should avoid that.
          //     Could hide it on a symbol on the event, or put out of band.
      ...
      // Inferred trans.
      const TRY_INFERRED_TRANS = true
      let inferredTrans = null
      if (TRY_INFERRED_TRANS) {
        if (triggerType === TRIGGER_API_GATEWAY) {
          const startTime = (event.version === '2.0'
            ? event.requestContext.timeEpoch
            : event.requestContext.requestTimeEpoch)
          assert(startTime, 'startTime')
          // XXX If there is a parent-id used for the API gateway span in X-Ray, then should we re-use it here?
          inferredTrans = agent.startTransaction(`AWS API Gateway: ${event.requestContext.domainName}`, 'request', {
            childOf: traceparent,
            tracestate: tracestate,
            startTime
          })
          traceparent = inferredTrans.traceparent
        }
      }
      ...
      wrapContext(trans, event, context, triggerType, inferredTrans)
      if (typeof callback === 'function') {
        callback = wrapLambdaCallback(trans, event, context, triggerType, inferredTrans, callback)
      }
