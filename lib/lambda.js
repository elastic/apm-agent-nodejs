...
function elasticApmAwsLambda(agent) {
  ...
  return function wrapLambdaHandler(type, fn) {
    ...
    console.log('XXX wrapLambdaHandler: fn.length: ', fn.length);

    // return async function wrappedLambdaHandler(event, context, callback) {
    // XXX v24: will this mask a user *having* a handler with a callback??
    //      I.e. should we error the same or pass through unchanged? How about we set wrappedFn.length = fn.length? TODO: try it.
    return async function wrappedLambdaHandler(event, context) {
      console.log('XXX wrappedLambdaHandler: start');

      //XXX hack for v24
      var callback = () => {};
      assert.ok(typeof arguments[2] !== 'function', 'XXX zomg, there is a callback? ' + arguments[2])

      if (!(event && context && typeof callback === 'function')) {
        // Skip instrumentation if arguments are unexpected.
        // https://docs.aws.amazon.com/lambda/latest/dg/nodejs-handler.html
        console.log('XXX wrappedLambdaHandler: skip instr');
      ...
      // XXX is `context.*()` all changed with v24?
      ...
      if (IS_V24) {
          const retval = ins.withRunContext(
            ...
            event,
            context,
            // XXX nope with v24: wrappedCallback, hack-called below in .then().
          );
          assert.ok(retval instanceof Promise, 'XXX wut, not a promise? retval='+retval)
          retval
            .then((...args) => {
              console.log('XXX handler success:', args);
              wrappedCallback(...args);
            })
            .catch((err) => {
              console.log('XXX handler threw err: ', err);
              wrappedCallback(err)
            });
...
// Returns the full file path to the user's handler module.
//
// TODO: support "extensionless"? per https://github.com/aws/aws-lambda-nodejs-runtime-interface-client/blob/v3.2.1/src/UserFunction.js#L149
//
...
function getFilePath(taskRoot, moduleRoot, module) {
  const basePath = path.resolve(taskRoot, moduleRoot, module);
