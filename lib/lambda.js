...
function setLambdaTransactionData(transaction, event, payload, isColdStart) {
  const faas = {
  ...
  // cloud. fields -- are transactions going to support these in the future?
...
function elasticApmAwsLambda (agent) {
  ...
  return function wrapLambda (type, fn) {
    ...
    return function wrappedLambda (event, context, callback) {
      ...
      const isColdStart = isFirstRun ? true : false
      if(isFirstRun) {
        isFirstRun = false
      }
