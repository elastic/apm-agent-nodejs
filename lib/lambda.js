...
let isFirstRun = true
let faasId // Set on first invocation.
...
function getLambdaMetadata (cloudAccountId) {
  return {
    service: {
...
function setApiGatewayData (trans, event, context, faasId, isColdStart) {
...
function setLambdaTransactionData (agent, trans, event, context, faasId, isColdStart) {
  if (isApiGatewayEvent(event)) {
    setApiGatewayData(trans, event, context, faasId, isColdStart)
...
function elasticApmAwsLambda (agent) {
  ...
  return function wrapLambda (type, fn) {
    ...
    return function wrappedLambda (event, context, callback) {
      ...
      setLambdaTransactionData(agent, trans, event, context, faasId, isColdStart)
