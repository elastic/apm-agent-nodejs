...
function getFilePathFromHandlerParts (taskRoot, handlerModule, pathParts) {
  let basePath = `${taskRoot}/${handlerModule}`
  if (pathParts.length > 0) {
    basePath = `${basePath}/${pathParts.join('/')}`
  }
  // if .js file doesn't exist, presume a cjs file
  let filePath = `${basePath}.js`
  if (!fs.existsSync(filePath)) {
    filePath = `${basePath}.cjs`
  }
  return filePath
}
...
function getLambdaHandlerInfo (env, modules, logger) {
  ...
  const pathParts = handlerFunctionPath.split('.')
  const handlerFunction = pathParts.pop()

  const handlerFilePath = getFilePathFromHandlerParts(env.LAMBDA_TASK_ROOT, handlerModule, pathParts)
