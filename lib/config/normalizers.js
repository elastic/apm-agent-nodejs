...
function normalizeKeyValuePair (value, key, options, logger) {
...
function normalizeNumber (value, key, options, logger) {
  return Number(value)
}
 ...
 * @param {Logger} logger
 */
function normalizeInfinities (opts, fields, defaults, logger) {
  for (const key of fields) {
    if (opts[key] === -1) opts[key] = Infinity
  }
}

function normalizeInfinity (value, key, options, logger) {
...
function normalizeByte (value, key, options, logger) {
...
function normalizeDuration (defaultUnit, allowedUnits, allowNegative) {
  return function (value, key, options, logger) {
    const val = secondsFromDuration(value, defaultUnit, allowedUnits, allowNegative)

    // TODO: work with defaults or invalid value
...
function normalizeArray (value, key, options, logger) {
...
function normalizeBool (value, key, options, logger) {
  return strictBool(logger, key, value)
}
...
function extractWildcards (sourceKey) {
  return function wildcardsExtractor (value, key, options, logger) {
    const source = options[sourceKey]
    const wildcard = new WildcardMatcher()
    const list = []

    for (const ptn of source) {
      const re = wildcard.compile(ptn)
      list.push(re)
    }
...
function extractStrings (sourceKey) {
  return function stringsExtractor (value, key, options, logger) {
    const source = options[sourceKey]
    const list = []
...
function extractRegExps (sourceKey) {
  return function regExpExtractor (value, key, options, logger) {
    const source = options[sourceKey]
    const list = []

    for (const item of source) {
      if (item instanceof RegExp) list.push(item)
    }
...
function normalizeSelect (allowedValues) {
  return function checkSelection (value, key, options, logger) {
    if (allowedValues.indexOf(value) === -1) {
      logger.warn('Invalid "%s" config value %s, falling back to default %s',
...
function normalizeSortedArray (value, key, options, logger) {
  let errReason = null
  if (!Array.isArray(value)) {
    errReason = 'value is not an array'
  } else if (value.some(el => typeof el !== 'number' || isNaN(el))) {
    errReason = 'array includes non-numbers'
  } else {
  ...
  if (errReason) {
    logger.warn('Invalid "%s" config value %j, %s; falling back to default',
      key, value, errReason)
    return options[key]
  }
...
function normalizeSampleRate (value, key, options, logger) {
  // The value was already run through `Number(...)` in `normalizeNumber`.
