...
class Config {
  constructor (opts, logger) {
    ...
    if (confFile === null && opts && opts.configFile) {
      ...
      configFilePath = confFile && path.resolve(opts.configFile)
    } else if (confFile) {
      configFilePath = path.resolve(process.env.ELASTIC_APM_CONFIG_FILE || 'elastic-apm-node.js')
    }
    ...
    // Print logging preamble now that the logger & all config is resolved
    const sources = [
      { name: 'environment', opts: envOptions },
      { name: 'start', opts: opts },
      { name: 'file', opts: confFile || {} }
    ]
    // printLoggingPreamble(this, sources, configFilePath)
    this.agentEnvConfigData = getAgentEnvConfigData(this, sources, configFilePath)
  }
...
function printLoggingPreamble (config, sources, configFilePath) {
  const { logger } = config
  const optsFromSources = sources.reduce((prev, s) => Object.assign(s.opts, prev), {})
  const isPreview = AGENT_VERSION.indexOf('-') !== -1
  const preamble = {
  ...
  Object.keys(optsFromSources).forEach(key => {
    if (key in EXCLUDE_FIELDS) {
      return
    }

    const shouldRedact = key in REDACT_FIELDS
    const sourceName = sources.find(s => (key in s.opts)).name
    preamble.config[key] = {
      source: sourceName,
      sourceValue: shouldRedact ? REDACTED : optsFromSources[key],
      normalizedValue: shouldRedact ? REDACTED : config[key]
    }
    if (sourceName === 'file') {
      preamble.config[key].file = configFilePath
    }
  })

  logger.info(preamble, 'Elastic APM Node.js Agent, version: %s', AGENT_VERSION)

  if (isPreview) {
    logger.warn('Version %s is a pre-release and not intended for use in production environments', AGENT_VERSION)
  }
}

function getAgentEnvConfigData (config, sources, configFilePath) {
  const optsFromSources = sources.reduce((prev, s) => Object.assign(s.opts, prev), {})
  const result = {
    agentVersion: AGENT_VERSION,
    env: {
  ...
  const mandatoryKeys = ['serviceName', 'serviceVersion', 'serverUrl', 'logLevel']
  const configKeys = new Set(mandatoryKeys.concat(Object.keys(optsFromSources)))
  ...
  configKeys.forEach(key => {
    ...
    result.config[key] = {
      normalizedName: formatOptionKey(key),
      source: sourceName,
      sourceValue: formatOptionValue(optsFromSources[key], shouldRedact), //  shouldRedact ? REDACTED : optsFromSources[key],
      normalizedValue: formatOptionValue(config[key], shouldRedact) // shouldRedact ? REDACTED : config[key],
    }
    if (sourceName === 'file') {
      result.config[key].file = configFilePath
    }
  })
...
/**
 * Formats the config value to be printed in log. For now it quotes the string values
 *
 * @param {any} value The value to be formatted if it's a string
 * @param {Boolean} shouldRedact true if the value should be redacted
 * @returns {any}
 */
function formatOptionValue (value, shouldRedact) {
  if (shouldRedact) {
    return REDACTED
  }
...
/**
 * Transforms keys in camelCase format to snake_case format
 *
 * @param {String} key A string contining an object key
 * @returns {String} the key trasformed to snake_case
 */
