...
class Config {
  constructor (opts, logger) {
    ...
    // at this point we do not know if it was loaded at boot or from the config option

    const sources = [
      { name: 'environment', options: envOptions },
      { name: 'start', options: opts },
      { name: 'file', options: confFile }
      // { name: 'default', options: DEFAULTS }
    ]
    printBanner(logger, sources)
