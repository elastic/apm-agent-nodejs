...
function configLogger (opts) {
  ...
  if (process.env.ELASTIC_APM_LOGGER !== 'false') {
    customLogger = (
      (opts && opts.logger) ||
      (confFileOpts && confFileOpts.logger)
    )
  }
...
class Config {
  constructor (opts, logger) {
    ...
    this.loggingPreambleData = getLoggingPreambleData(this, sources, confFilePath)
  }
...
function loadConfigFile (filePath) {
  const confPath = path.resolve(filePath || process.env.ELASTIC_APM_CONFIG_FILE || 'elastic-apm-node.js')
...
function getLoggingPreambleData (config, sources, configFilePath) {
  ...
  configKeys.forEach(key => {
    ...
    if (shouldRedact) {
      optionDetails.value = key === 'serverUrl' ? sanitizeUrl(config[key]) : REDACTED
    ...
    if (key in CROSS_AGENT_CONFIG_VAR_NAME) {
      result.config[key].commonName = CROSS_AGENT_CONFIG_VAR_NAME[key]
    }
    if (sourceName === 'file') {
 ...
 * @returns {String} url with basic auth REDACTED
 */
function sanitizeUrl (urlStr) {
  ...
  if (url.username || url.password) {
    result = result.replace(url.username, REDACTED)
    result = result.replace(url.password, REDACTED)
  }
