...
class Config {
  constructor (opts, logger) {
    ...
    if (confFile === null && opts && opts.configFile) {
      ...
      configFilePath = confFile && path.resolve(opts.confFile)
    } else if (confFile) {
    ...
    // Print logging preamble now that the logger is completelly resolved
    const sources = {
...
function printLoggingPreamble (config, sources, configFilePath) {
  const { logger } = config
  const nonDefaultKeys = ['environment', 'start', 'file']
  const nonDefaultOpts = Object.assign.apply(null, nonDefaultKeys.map(k => sources[k]).reverse())
  const isPreview = AGENT_VERSION.indexOf('-') !== -1

  // Version and pre-release warning
  logger.info(`Elastic APM Node.js Agent, version: ${AGENT_VERSION}`)

  if (isPreview) {
    logger.warn('This is a pre-release version and not intended for use in production environments!')
  }
  ...
  const timeZone = `UTC${offsetSign}${offsetPad}${Math.abs(offsetHours * 100)}`
  const frameWorkName = nonDefaultOpts.frameWorkName || 'unknown'
  const frameWorkVersion = nonDefaultOpts.frameWorkVersion || ''
  const environmentMessage = [
    ...
    // sometimes this info is delayed (maybe not be available)
    `- Framework: ${frameWorkName} ${frameWorkVersion}`
  ]

  logger.info(environmentMessage.join('\n'))
  ...
  for (const optKey in nonDefaultOpts) {
    if (EXCLUDE_FIELDS[optKey]) {
      continue
    }

    // const origin = nonDefaultKeys.reduce((orig, key) => (optKey in sources[key]) ? key : orig, '')
    const origin = nonDefaultKeys.find(key => (optKey in sources[key]))
    let value

    if (REDACT_FIELDS[optKey]) {
      value = REDACTED
    } else if (NICE_REGEXPS_FIELDS[optKey] && Array.isArray(nonDefaultOpts[optKey])) {
      // JSON.stringify() on a RegExp is "{}", which isn't very helpful.
      value = nonDefaultOpts[optKey].map(r => r instanceof RegExp ? r.toString() : r)
    } else {
      value = nonDefaultOpts[optKey]
    }

    optionsMessage.push(`- ${optKey}: ${value.toString()} (${origin})`)
  }

  logger.info(optionsMessage.join('\n'))
}
