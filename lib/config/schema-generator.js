...
DURATION_OPTS.forEach((spec) => {
  ...
  def.normalizers = def.normalizers || []
  def.normalizers.push(`durationFn('${defaultUnit}', [${unitsStr}], ${allowNegative})`)
...
const schemaSource = schemaText.split('\n').map((line) => {
  ...
  line = line.replace(/"durationFn\((.+)\)"/, (match, group1) => `durationFn(${group1})`)
  line = line.replace(/"/g, "'")
  return line
})
