...
const ALL_DEFAULTS = Object.assign({}, DEFAULTS, MISSING_DEFAULTS)
const schemaObj = Object.keys(ALL_DEFAULTS).reduce((acc, key) => {
...
const durationTypes = Object.entries({
  durationSeconds: 's|ms,s,m|false',
  durationSecondsNegative: 's|ms,s,m|true',
  durationMiliseconds: 'ms|us,ms,s,m|false',
  durationCompression: 'ms|ms,s,m|false',
  durationMinSeconds: 's|ms,s,m|true',
  durationMilisecondsNegative: 'ms|ms,s,m|true'
})
DURATION_OPTS.forEach((spec) => {
  const { name, defaultUnit, allowedUnits, allowNegative } = spec
  const def = schemaObj[name] || {}
  const typeEntry = durationTypes.find(dt => dt[1] === `${defaultUnit}|${allowedUnits}|${allowNegative}`)

  if (!typeEntry) {
    throw Error('no type found for duration spec', spec)
  }
  def.name = name
  def.type = typeEntry[0]
})
