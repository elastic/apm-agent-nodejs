...
function truncTransactionPayload (payload) {
  payload.transactions.forEach(function (trans) {
    ...
    trans.traces.forEach(function (trace) {
      trace.name = trunc(String(trace.name), config.INTAKE_STRING_MAX_SIZE)
      trace.type = trunc(String(trace.type), config.INTAKE_STRING_MAX_SIZE)
...
function envelope (agent) {
  var payload = {
    app: {
      ...
      environment: trunc(String(agent._conf.environment), config.INTAKE_STRING_MAX_SIZE)
    },
    system: {
