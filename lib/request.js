...
function envelope (agent) {
  var payload = {
    service: {
      ...
      runtime: {
        name: truncate(String((process.release && process.release.name) || process.argv[0]), config.INTAKE_STRING_MAX_SIZE), // process.release was introduced in v3.0.0
        version: truncate(String(process.version), config.INTAKE_STRING_MAX_SIZE)
      },
      language: {
        name: 'javascript'
      },
    ...
    system: {
      ...
      architecture: truncate(String(process.arch), config.INTAKE_STRING_MAX_SIZE),
      platform: truncate(String(process.platform), config.INTAKE_STRING_MAX_SIZE)
    }
  }
