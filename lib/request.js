...
function truncErrorData (data) {
  ...
  if (data.exception && data.exception.value) {
    data.exception.value = trunc(data.exception.value, 2048)
  }

  if (data.culprit) {
    data.culprit = trunc(data.culprit, 100)
  }

  if (data.message) {
    data.message = trunc(data.message, 200)
  }
}
...
function truncTransactionData (data) {
  if (data.transactions) {
    data.transactions.forEach(function (trans) {
      trans.transaction = trunc(trans.transaction, 512)
    })
  }

  if (data.traces && data.traces.groups) {
    data.traces.groups.forEach(function (group) {
      if (group.transaction) group.transaction = trunc(group.transaction, 512)
...
function truncFrames (frames) {
  ...
  frames.forEach(function (frame, i) {
    ...
    if (frame.context_line) frame.context_line = trunc(frame.context_line, 1000)
