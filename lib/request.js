...
exports._ES_KEYWORD_MAX_SIZE = ES_KEYWORD_MAX_SIZE
...
function truncErrorsPayload (payload) {
  payload.errors.forEach(function (error) {
    if (error.log) {
      if (error.log.level) {
        error.log.level = trunc(String(error.log.level), ES_KEYWORD_MAX_SIZE)
      }
      if (error.log.logger_name) {
        error.log.logger_name = trunc(String(error.log.logger_name), ES_KEYWORD_MAX_SIZE)
      }
      if (error.log.param_message) {
        error.log.param_message = trunc(String(error.log.param_message), ES_KEYWORD_MAX_SIZE)
      }
    ...
    if (error.exception) {
      if (error.exception.type) {
        error.exception.type = trunc(String(error.exception.type), ES_KEYWORD_MAX_SIZE)
      }
      if (error.exception.code) {
        error.exception.code = trunc(String(error.exception.code), ES_KEYWORD_MAX_SIZE)
      }
      if (error.exception.module) {
        error.exception.module = trunc(String(error.exception.module), ES_KEYWORD_MAX_SIZE)
      }
...
function truncTransactionPayload (payload) {
  payload.transactions.forEach(function (trans) {
    trans.name = trunc(String(trans.name), ES_KEYWORD_MAX_SIZE)
    trans.type = trunc(String(trans.type), ES_KEYWORD_MAX_SIZE)
    trans.result = trunc(String(trans.result), ES_KEYWORD_MAX_SIZE)

    trans.traces.forEach(function (trace) {
      trace.name = trunc(String(trace.name), ES_KEYWORD_MAX_SIZE)
      trace.type = trunc(String(trace.type), ES_KEYWORD_MAX_SIZE)
...
function truncContext (context) {
  ...
  if (context.request) {
    if (context.request.method) {
      context.request.method = trunc(String(context.request.method), ES_KEYWORD_MAX_SIZE)
    }
    if (context.request.url) {
      if (context.request.url.protocol) {
        context.request.url.protocol = trunc(String(context.request.url.protocol), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.hostname) {
        context.request.url.hostname = trunc(String(context.request.url.hostname), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.port) {
        context.request.url.port = trunc(String(context.request.url.port), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.pathname) {
        context.request.url.pathname = trunc(String(context.request.url.pathname), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.search) {
        context.request.url.search = trunc(String(context.request.url.search), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.hash) {
        context.request.url.hash = trunc(String(context.request.url.hash), ES_KEYWORD_MAX_SIZE)
      }
      if (context.request.url.raw) {
        context.request.url.raw = trunc(String(context.request.url.raw), ES_KEYWORD_MAX_SIZE)
      }
    }
  }
  if (context.user) {
    if (context.user.id) {
      context.user.id = trunc(String(context.user.id), ES_KEYWORD_MAX_SIZE)
    }
    if (context.user.email) {
      context.user.email = trunc(String(context.user.email), ES_KEYWORD_MAX_SIZE)
    }
    if (context.user.username) {
      context.user.username = trunc(String(context.user.username), ES_KEYWORD_MAX_SIZE)
    }
  }
}
...
function envelope (agent) {
  var payload = {
    app: {
      name: agent.appName,
      pid: process.pid,
      process_title: trunc(String(process.title), ES_KEYWORD_MAX_SIZE),
      argv: process.argv,
      runtime: {
        name: trunc(String((process.release && process.release.name) || process.argv[0]), ES_KEYWORD_MAX_SIZE), // process.release was introduced in v3.0.0
        version: trunc(String(process.version), ES_KEYWORD_MAX_SIZE)
      },
      agent: {
        name: 'nodejs',
