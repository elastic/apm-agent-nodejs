...
function GenericSpan (agent, ...args) {
  ...
  // XXX API changes I'd *like* for span creation here:
  // - follow OTel lead and pass through a RunContext rather than the various
  //   types `childOf` can be. That RunContext identifies a parent span (or
  //   none if the RunContext).
  // - Conversion of `childOf` to RunContext should move to the caller of `new
  //   {Transaction|Span}` instead of in the ctor here.
  // this._traceContext = TraceContext.startOrResume(opts.childOf, agent._conf, opts.tracestate)
