...
GenericSpan.prototype.duration = function () {
  if (!this.ended) {
    this._agent.logger.debug({ id: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type }, 'tried to call duration() on un-ended transaction/span')
    return null
  }
