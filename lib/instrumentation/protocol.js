...
function groupTransactions (samples, durations) {
  ...
  return Object.keys(groups).map(function (key) {
    ...
    return {
      ...
      result: trans.result,
      kind: trans.type,
      timestamp: new Date(groupingTs(trans._timer.start)).toISOString(),
...
function groupTraces (traces) {
  ...
  return Object.keys(groups).map(function (key, index) {
    ...
    return {
      ...
      timestamp: new Date(groupingTs(trace._timer.start)).toISOString(),
      parents: [],
      extra: trace.extra
    }
  })
...
function rawTraces (traces) {
  return traces.map(function (trace) {
    return [trace._groupIndex, trace.offsetTime(), trace.duration()]
  })
}
...
function transactionGroupingKey (trans) {
  return groupingTs(trans._timer.start) +
    '|' + trans.name +
    '|' + trans.result +
    '|' + trans.type
}
...
function traceGroupingKey (trace) {
  return groupingTs(trace.transaction._timer.start) +
