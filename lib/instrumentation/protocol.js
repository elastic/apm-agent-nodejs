...
function encode (transactions, cb) {
  ...
  var next = afterAll(function (err, frames) {
    ...
    cb(null, transactions.map(function (trans, index) {
      ...
      if (trans.sampled) {
        payload.spans = encodeSpans(trans.spans, frames[index])
        payload.context = {
          user: Object.assign(
            {},
      ...
      if (trans._droppedSpans > 0) {
        payload.span_count = {
          dropped: {
            total: trans._droppedSpans
          }
        }
      }
  ...
  if (transactions[0]._agent._conf.captureSpanStackTraces) {
