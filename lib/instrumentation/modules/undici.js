...
// an esbuild bundle. This instrumentation is enabled if either Node >=v18 is
// used or `require('undici')`.
//
// Limitations:
...
// If the header is no present, or has an invalid value, this returns null.
...
function instrumentUndici (agent) {
  ...
  diagchSub('undici:request:create', ({ request }) => {
    ...
    const span = ins.createSpan(`${request.method} ${host}`, 'external', 'http', request.method, { exitSpan: true })
  ...
  diagchSub('undici:request:headers', ({ request, response }) => {
    ...
    if (span !== undefined) {
      // We are currently *not* capturing response headers, though the intake
      // API does allow it, because none of the other `setHttpContext` users do.
...
function shimUndici (undici, agent, { version, enabled }) {
  ...
  if (semver.lt(version, '4.7.0')) {
    agent.logger.debug('cannot instrument undici: undici version %s is not supported', version)
    return undici
  }
  if (!diagch) {
    agent.logger.debug('cannot instrument undici: node %s does not support diagnostics_channel', process.version)
    return undici
  }
