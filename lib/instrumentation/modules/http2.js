...
module.exports = function (http2, agent, { enabled }) {
  ...
  function wrapEnd (original) {
    return function (headers) {
      var trans = agent._instrumentation.currTransaction()
      // Before https://github.com/nodejs/node/pull/20084 the 'end' event could
      // be called multiple times for the same Http2Stream, and the `trans.res`
      // references is removed when the Transaction is ended.
      if (trans && trans.res) {
