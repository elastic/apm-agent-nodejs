...
module.exports = function (modExports, agent, { enabled, isImportMod }) {
  if (agent._conf.instrumentIncomingHTTPRequests) {
    ...
    shimmer.wrap(modExports && modExports.ServerResponse && modExports.ServerResponse.prototype,
      'writeHead',
      wrapWriteHead)
  }
  ...
  let wrapped = shimmer.wrap(modExports, 'request', httpShared.traceOutgoingRequest(agent, 'http', 'request'))
  ...
  wrapped = shimmer.wrap(modExports, 'get', httpShared.traceOutgoingRequest(agent, 'http', 'get'))
  if (isImportMod && wrapped) {
