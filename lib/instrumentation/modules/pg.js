...
function patchClient (Client, klass, agent, enabled) {
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      const [type, subType] = ['db', 'postgresql']
      var span = agent.startSpan('SQL', type, subType, 'query')
      ...
      if (span) {
        ...
        const [host, port] = [process.env.PGHOST, process.env.PGPORT]
        span.setDestinationContext(getDBDestination(host, port, type, subType))
