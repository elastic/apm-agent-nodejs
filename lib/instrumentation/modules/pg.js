...
function patchClient (Client, klass, agent, enabled) {
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      ...
      agent.logger.debug({ id, sql }, 'intercepted call to %s.prototype.%s', klass, name)
      ...
      if (span) {
        ...
        if (typeof cb === 'function') {
          ...
          if (typeof query.on === 'function') {
            ...
            agent.logger.debug({ id }, 'ERROR: unknown pg query type: %s', typeof query)
          }
      ...
      function end () {
        agent.logger.debug({ id }, 'intercepted end of %s.prototype.%s', klass, name)
        span.end()
