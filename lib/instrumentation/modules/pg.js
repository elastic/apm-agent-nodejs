...
function patchClient (Client, klass, opbeat) {
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      var trans = opbeat.activeTransaction()
      var uuid = trans ? trans._uuid : 'n/a'

      opbeat.logger.trace('[%s] intercepted call to %s.prototype.%s (transaction: %s, sql: %s)', uuid, klass, name, trans ? 'exists' : 'missing', sql)
