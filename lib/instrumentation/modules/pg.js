...
function patchClient (Client, klass, opbeat) {
  logger.trace('shimming %s.prototype.query', klass)
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      ...
      logger.trace('[%s] intercepted call to %s.prototype.%s (transaction: %s, sql: %s)', uuid, klass, name, trace ? 'exists' : 'missing', sql)
      ...
      function end () {
        logger.trace('[%s] intercepted end of %s.prototype.%s', uuid, klass, name)
        trace.end()
