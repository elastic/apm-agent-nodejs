    ...
    return function wrappedQuery (sql, values, cb) {
      ...
      if (span) {
        ...
        const wrapCallback = function (origCallback) {
          hasCallback = true
          return ins.bindFunction(function wrappedCallback (err) {
            if (err) {
              console.warn('XXX TODO: setOutcome, captureError? for mysql err')
            }
            span.end()
      ...
      if (span && !hasCallback && result instanceof EventEmitter) {
        ...
        shimmer.wrap(result, 'emit', function (origEmit) {
          return function wrappedEmit (event, data) {
            ...
            switch (event) {
              case 'error':
                // XXX TODO: tests for error cases, e.g. 'SELECT 1 + AS solution'
                console.warn('XXX TODO: setOutcome from mysql error event: %s', data)
                break
              case 'end':
                // XXX Note we get _recoverTransaction hits here, but it shouldn't matter
                //     because span.end() doesn't run any code that needs to execute in
                //     the run context for this span/transaction.
                span.end()
                break
            }
