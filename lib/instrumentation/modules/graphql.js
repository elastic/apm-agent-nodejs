...
const getPathFromRequest = require('../express-utils').getPathFromRequest
...
module.exports = function (graphql, agent, { version, enabled }) {
      ...
      if (sourceObj) {
        ...
        if (documentAST) {
          ...
          if (validationErrors && validationErrors.length === 0) {
            const queries = extractDetails(documentAST, operationName).queries
  ...
  function wrapExecute (orig) {
    return function wrappedExecute (args) {
      ...
      const details = extractDetails(document, operationName)
      ...
      if (trans._graphqlRoute) {
        const name = queries.length > 0 ? queries.join(', ') : 'Unknown GraphQL query'
  ...
  function extractDetails (document, operationName) {
    ...
    if (document && Array.isArray(document.definitions)) {
      ...
      const selections = operation && operation.selectionSet && operation.selectionSet.selections
