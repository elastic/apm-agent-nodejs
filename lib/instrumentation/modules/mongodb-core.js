...
module.exports = function (mongodb, agent, { version, enabled }) {
  ...
  function wrapCommand (orig) {
    return function wrappedFunction (ns, cmd) {
      ...
      agent.logger.debug({ id, ns }, 'intercepted call to mongodb-core.Server.prototype.command')
      ...
      function wrappedCallback () {
        agent.logger.debug({ id }, 'intercepted mongodb-core.Server.prototype.command callback')
        span.end()
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (ns) {
      ...
      agent.logger.debug({ id, ns }, 'intercepted call to mongodb-core.Server.prototype.%s', name)
      ...
      function wrappedCallback () {
        agent.logger.debug({ id }, 'intercepted mongodb-core.Server.prototype.%s callback', name)
        span.end()
        return cb.apply(this, arguments)
      }
    }
  ...
  function wrapCursor (orig, name) {
    return function wrappedFunction () {
      ...
      agent.logger.debug({ id }, 'intercepted call to mongodb-core.Cursor.prototype.%s', name)
      ...
      function wrappedCallback () {
        agent.logger.debug({ id }, 'intercepted mongodb-core.Cursor.prototype.%s callback', name)
        span.end()
        return cb.apply(this, arguments)
      }
    }
