...
module.exports = function (mongodb, agent) {
  ...
  function wrapCommand (orig) {
    return function wrappedFunction (ns, cmd) {
      ...
      agent.logger.trace('[%s] intercepted call to mongodb-core.Server.prototype.command (transaction: %s, ns: %s)', uuid, trace ? 'exists' : 'missing', ns)
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (ns) {
      ...
      agent.logger.trace('[%s] intercepted call to mongodb-core.Server.prototype.%s (transaction: %s, ns: %s)', uuid, name, trace ? 'exists' : 'missing', ns)
  ...
  function wrapCursor (orig, name) {
    return function wrappedFunction () {
      ...
      agent.logger.trace('[%s] intercepted call to mongodb-core.Cursor.prototype.%s (transaction: %s)', uuid, name, trace ? 'exists' : 'missing')
