...
module.exports = function (mongodb, agent) {
  ...
  function wrapCommand (orig) {
    return function wrappedFunction (ns, cmd) {
      ...
      debug('intercepted call to mongodb-core.Server.prototype.command', { uuid: uuid, ns: ns })
      ...
      function wrappedCallback () {
        debug('intercepted mongodb-core.Server.prototype.command callback', { uuid: uuid })
        trace.end()
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (ns) {
      ...
      debug('intercepted call to mongodb-core.Server.prototype.%s', name, { uuid: uuid, ns: ns })
      ...
      function wrappedCallback () {
        debug('intercepted mongodb-core.Server.prototype.%s callback', name, { uuid: uuid })
        trace.end()
        return cb.apply(null, arguments)
      }
    }
  ...
  function wrapCursor (orig, name) {
    return function wrappedFunction () {
      ...
      debug('intercepted call to mongodb-core.Cursor.prototype.%s', name, { uuid: uuid })
      ...
      function wrappedCallback () {
        debug('intercepted mongodb-core.Cursor.prototype.%s callback', name, { uuid: uuid })
        trace.end()
        return cb.apply(null, arguments)
      }
    }
