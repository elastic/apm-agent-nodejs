  ...
  return [
    {
      middleware: (next, context) => async (args) => {
        ...
        if (parentSpan) {
          const actionName = context.commandName.replace('Command', '');
          const toPropagate = [];

          if (actionName === 'SendMessage' && input.MessageAttributes) {
            toPropagate.push(input.MessageAttributes);
          } else if (
            actionName === 'SendMessageBatch' &&
            Array.isArray(input.Entries)
          ) {
            toPropagate.push(...input.Entries);
          }
        ...
        span.action = OPERATIONS_TO_ACTIONS[span.action] || 'unknown';
