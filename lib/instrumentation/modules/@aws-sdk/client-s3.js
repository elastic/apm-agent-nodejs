    ...
    constructor (...args) {
      ...
      this.middlewareStack.add(
        (next, context) => async (args) => {
          console.warn('XXX context at finalizeRequest time', context)
          console.warn('XXX args at finalizeRequest time', args)
          // XXX set destination context here,
          //     what was `hostname: 's3.us-west-1.amazonaws.com'` at build
          //     time is now `hostname: 'trentm-play-s3-bukkit2.s3.us-west-1.amazonaws.com',`
      ...
      // this.middlewareStack.add(
      //   (next, context) => async (args) => {
      //     console.warn('XXX context at build time', context)
      //     console.warn('XXX args at build time', args)
      //     return await next(args)
      //   },
      //   {
      //     step: 'build'
      ...
      this.middlewareStack.add(
        (next, context) => async (args) => {
          ...
          try {
            ...
            // XXX I don't think this catches error path (e.g. with ExpiredToken). try/catch for that?!
            if (span) {
              if (err) {
                // XXX captureError, depending in statusCode again
              }

              // XXX region technique from ruby might be good here:
              //    - if client.conf.useArnRegion, then get from Arn if it is, else fallback to client.conf.region?
              console.warn('XXX result', result)
              // console.warn('XXX client.config.region', client.config.region())
              const region = await client.config.region()
              console.warn('XXX region:', region)

              // XXX Destination context.
