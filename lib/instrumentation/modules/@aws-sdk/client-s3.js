  ...
  return [
    {
      middleware: (next, context) => async (args) => {
        ...
        const span = agent._instrumentation.createSpan(name, TYPE, SUBTYPE, opName, { exitSpan: true })
        ...
        try {
          ...
          if (span) {
            ...
            if (statusCode) {
              ...
              // const httpResponse = result && result.response
              const httpContext = {
                status_code: statusCode
              }

              // TODO: check if possible to get the size
              // const encodedBodySize = httpResponse && httpResponse.readableLength
              // if (encodedBodySize) {
