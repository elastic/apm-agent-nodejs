...
const clientsConfig = {
  S3Client: {
    ...
    TYPE: 'storage',
    SUBTYPE: 's3',
    middlewares: createMiddlewaresForS3Client
  }
}
  ...
  shimmer.wrap(mod.Client.prototype, 'send', function (orig) {
    return function _wrappedSmithyClientSend () {
      ...
      if (!this[elasticAPMMiddlewares]) {
        const factory = clientConfig && clientConfig.middlewares
      ...
      if (clientConfig) {
        ...
        const name = clientConfig.prefix + ' ' + opName

        const ins = agent._instrumentation
        const span = ins.createSpan(name, clientConfig.TYPE, clientConfig.SUBTYPE, opName, { exitSpan: true })
        ...
        if (typeof cb === 'function') {
          arguments[arguments.length - 1] = ins.bindFunctionToRunContext(parentRunContext, cb)
        }
