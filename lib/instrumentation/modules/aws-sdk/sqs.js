...
function propagateTraceContextInMessageAttributes (span, msgParams, log, queueUrl) {
  if (!msgParams.MessageAttributes) {
    msgParams.MessageAttributes = {}
  }
  const msgAttrs = msgParams.MessageAttributes
  if (Object.keys(msgAttrs).length + 2 > MAX_SQS_MESSAGE_ATTRIBUTES) {
    log.warn({ QueueUrl: queueUrl },
      'cannot propagate trace-context with SQS message, too many MessageAttributes')
  } else {
    span.propagateTraceContextHeaders(msgAttrs, function (msgAttrs, name, value) {
...
function instrumentationSqs (orig, origArguments, request, AWS, agent, { version, enabled }) {
  ...
  // XXX do this even if !span (see http-shared for example)
  if (!request.params || typeof request.params !== 'object') {
    // Pass. This is a sanity guard against bogus user input.
  } else if (request.operation === 'sendMessage') {
    propagateTraceContextInMessageAttributes(span, request.params, log, request.params.QueueUrl)
  } else if (request.operation === 'sendMessageBatch' && Array.isArray(request.params.Entries)) {
    request.params.Entries.forEach(msgParams => {
      propagateTraceContextInMessageAttributes(span, msgParams, log, request.params.QueueUrl)
    })
  }
