...
function instrumentationS3 (orig, origArguments, request, AWS, agent, { version, enabled }) {
  ...
  // XXX whoa this is wrong, `startSpan()` parent should be the current span-or-tx
  //     and NOT just the currTransaction. TODO: test case for this perhaps.
  const span = ins.currTransaction().startSpan(name, TYPE, SUBTYPE, opName)
  if (!span) {
  ...
  function onComplete (response) {
    ...
    // XXX can we drop this issue (here and in other aws-sdk instrumentations) with the _startXid-based impl?
  ...
  // XXX I don't like `enterSpan` name here, perhaps `newWithSpan()`?
  // XXX I don't like having to reach into _runCtxMgr. It should be an API on Instrumentation.
