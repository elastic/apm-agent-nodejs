...
function instrumentationS3 (orig, origArguments, request, AWS, agent, { version, enabled }) {
  ...
  // the S3 span.
  const parentContext = ins._runCtxMgr.active()
  const runContext = parentContext.enterSpan(span)
  const cb = origArguments[origArguments.length - 1]
  if (typeof cb === 'function') {
    origArguments[origArguments.length - 1] = ins._runCtxMgr.bindFunction(parentContext, cb)
  }
