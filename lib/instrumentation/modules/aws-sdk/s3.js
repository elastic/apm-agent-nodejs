...
function instrumentationS3 (orig, origArguments, request, AWS, agent, { version, enabled }) {
  ...
  // XXX API access to internal things. Clean up this API to be on Instrumentation
  const runContext = ins._runCtxMgr.active().enterSpan(span)
  // XXX For trace-s3.js parentage cases to work, we need either or both of:
  //    - RunContext#currSpan skipping ended spans
  //    - `traceOutgoingRequest` *not* doing `ins.bindEmitter(res)` in `onresponse`
  //    I don't know, but I think the former is a fallback, and the latter was
  //    an earlier flail... maybe.
  //    And doing the former breaks "test/run-context/fixtures/parentage-with-ended-span.js"
  //    for better or worse. I'm not sure about that test. Real world case would be nice.
  //    I expect that living without `RunContext#currSpan skipping ended spans` will
  //    be a subtle game of whack-a-mole. Every `ins.bindEmitter` is suspect!
  // request.on('complete', ins._runCtxMgr.bindFunction(runContext, onComplete))
  request.on('complete', onComplete)
