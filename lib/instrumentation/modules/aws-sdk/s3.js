...
function instrumentationS3 (orig, origArguments, request, AWS, agent, { version, enabled }) {
  ...
  const span = ins.startSpan(name, TYPE, SUBTYPE, opName)
  if (span) {
    // A possibly given callback function in `origArguments` is called in the
    // 'complete' event. Binding the emitter to the current run context ensures
    // the callback and any user-added event handlers run in the context of
    // this span.
    ins.bindEmitter(request)
