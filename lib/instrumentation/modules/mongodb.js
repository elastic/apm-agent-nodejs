  ...
  function onStart (event) {
    ...
    if (span) {
      ...
      if (address && typeof (address) === 'string' &&
        ...
        span._setDestinationContext(getDBDestination(span, match[1], match[2]))
      } else {
        agent.logger.trace('could not set destination context on mongodb span from address=%j', address)
      }

      // DB context.
      // XXX TODO: resolve https://github.com/elastic/apm-agent-nodejs/issues/1494
      //    with this change: we'll add db.instance, and db.link isn't relevant
      //    per discussion on https://github.com/elastic/apm-agent-nodejs/issues/1482.
      // XXX Add a new issue to add mongodb span.context.db.statement.
      //    The thing to figure is how to create "The MongoDB command encoded as
      //    MongoDB Extended JSON." from `event.command` in mongodb.js.
