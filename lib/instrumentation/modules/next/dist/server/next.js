...
module.exports = function (mod, agent, { version, enabled }) {
  console.log('XXX hi in "next" instr (at next/dist/server/next.js)')
  if (!enabled) {
    return mod
  }
  ...
  if (typeof mod !== 'function' || mod.default !== mod) {
    agent.logger.debug('next module is not the expected form, skipping')
    return mod
  }
  ...
  // The top-level module `createServer()` returns a `NextServer` with a
  // `.createServer()` method that returns a promise for a next Server subclass
  // instance that we want to instrument.
  agent.logger.debug('wrapping next (createServer)')
  const origCreateServer = mod
  function wrappedModCreateServer () {
    const nextServerInst = origCreateServer.apply(this, arguments)
    console.log('XXX nextServerInst: ', nextServerInst.constructor.prototype)
    shimmer.wrap(nextServerInst.constructor.prototype, 'createServer', wrapCreateServer)
    return nextServerInst
  }
  wrappedModCreateServer.default = wrappedModCreateServer

  return wrappedModCreateServer

  function wrapCreateServer (orig) {
    return function wrappedCreateServer () {
      const promise = orig.apply(this, arguments)
      promise.then(serverInst => {
        // `serverInst` is an instance of next-server.ts `NextNodeServer` (when
        // using `next build && next start`) or next-dev-server.ts `DevServer`
        // (when using `next dev`).
        const prototype = serverInst.constructor.prototype
        console.log('XXX prototype: ', prototype.generateRoutes, serverInst.constructor)
        shimmer.wrap(prototype, 'generateRoutes', wrapGenerateRoutes)
        shimmer.wrap(prototype, 'ensureApiPage', wrapEnsureApiPage)
        shimmer.wrap(prototype, 'findPageComponents', wrapFindPageComponents)
        shimmer.wrap(prototype, 'renderErrorToResponse', wrapRenderErrorToResponse)
      })
      return promise
    }
  }
  ...
  function wrapGenerateRoutes (orig) {
    console.log('XXX wrapGenerateRoutes')
    return function wrappedGenerateRoutes () {
      if (this.constructor.name === 'DevServer') {
        console.log('  XXX do the dev server thing')
      }
          ...
          if (trans) {
            ...
            if (!trans[kSetTransNameFn]) {
              trans[kSetTransNameFn] = (_req, pathname) => {
                trans.setDefaultName(`Next.js _next/data for ${pathname}`)
              }
            }
          }
          return origRouteFn.apply(this, arguments)
