...
module.exports = function (mariadb, agent, { version, enabled, name }) {
  ...
  function wrapQuery(original, name) {
    ...
    return function wrappedQuery(sql, values, cb) {
      console.log('Executing wrapQuery', arguments);
      ...
      if (!hasCallback && name !== 'queryStream') {
