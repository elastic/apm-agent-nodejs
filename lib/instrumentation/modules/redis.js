...
module.exports = function (redis, agent, { version, enabled }) {
  ...
  function wrapInternalSendCommand (original) {
    return function wrappedInternalSendCommand (commandObj) {
      // Avoid re-wrapping internal_send_command called *again* in
      // send_offline_queue for commands queued before "ready".
      if (commandObj && commandObj.callback && commandObj.callback[isWrappedRedisCbSym]) {
