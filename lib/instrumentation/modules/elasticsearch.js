...
module.exports = function (elasticsearch, agent, { enabled }) {
  ...
  function wrapRequest (original) {
    return function wrappedRequest (params, cb) {
      const [type, subType] = ['db', 'elasticsearch']
      var span = agent.startSpan(null, type, subType, 'request')
      ...
      if (span && method && path) {
        ...
        if (queryRegexp.test(path)) {
          ...
          const esConfig = this._config
          let host
          let port
          if (esConfig && esConfig.host) {
            [host, port] = esConfig.host.split(':')
          }
          span.setDestinationContext(getDBDestination(type, subType, host, port))
        }
