...
module.exports = function (elasticsearch, agent, { enabled }) {
  ...
  function wrapRequest (original) {
    return function wrappedRequest (params, cb) {
      ...
      agent.logger.debug({ id, method, path }, 'intercepted call to elasticsearch.Transport.prototype.request')
      ...
      if (span && method && path) {
        ...
        agent.logger.debug({ id }, 'could not instrument elasticsearch request')
        return original.apply(this, arguments)
      }
    }
  }
