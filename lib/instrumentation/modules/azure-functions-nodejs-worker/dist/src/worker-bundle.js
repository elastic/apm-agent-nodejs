...
const httpRouteFromFuncDir = new Map()
...
// ...plus some additional functionality for `httpRoute`.
class FunctionInfo {
  constructor (bindingDefinitions, executionContext, log) {
    ...
    // If this is an HTTP triggered-function, then get its route.
    ...
    // attempt to load the "function.json" file.
    this.httpRoute = null
    if (this.hasHttpTrigger) {
      // XXX test this for realz, `functionDirectory` is not on the Azure interfaces for Java, etc. Exists in real Azure?
      const funcDir = executionContext.functionDirectory
      if (!funcDir) {
        this.httpRoute = executionContext.functionName
      } else if (httpRouteFromFuncDir.has(funcDir)) {
        this.httpRoute = httpRouteFromFuncDir.get(funcDir)
      } else {
        try {
          ...
          for (let i = 0; i < fj.bindings.length; i++) {
            ...
            if (binding.direction === 'in' && binding.type && binding.type.toLowerCase() === 'httptrigger') {
              ...
              httpRouteFromFuncDir.set(funcDir, this.httpRoute)
            }
          }
    ...
    core.registerHook('preInvocation', (hookCtx) => {
      ...
      if (triggerType === TRIGGER_HTTP) {
        ...
        if (agent._conf.usePathAsTransactionName && req.url) {
          ...
          trans.setDefaultName(`${req.method} ${funcInfo.httpRoute}`)
        }
      }
    })
