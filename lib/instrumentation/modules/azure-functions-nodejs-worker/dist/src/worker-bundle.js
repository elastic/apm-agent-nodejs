    ...
    core.registerHook('preInvocation', (hookCtx) => {
      ...
      if (triggerType === TRIGGER_HTTP) {
        ...
        if (req && req.url) {
          ...
          // XXX HERE:
          //    - use "route" in the binding if set
          //    - else, to allow extensions.http.routePrefix: ""` in "host.json"
          //    (or whatever prefix) how about we take the URL path, dirname
          //    that and append the `functionName` (to deal with case)
