...
module.exports = function (Knex, agent, { version, enabled }) {
  ...
  if (semver.satisfies(version, '>=1 <3') && semver.lt(process.version, '12.0.0')) {
    agent.logger.debug('knex version %s does not support node %s, skipping knex instrumentation', version, process.version)
    return Knex
  }
