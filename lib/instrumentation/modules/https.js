...
module.exports = function (modExports, agent, { version, enabled, isImportMod }) {
  if (agent._conf.instrumentIncomingHTTPRequests) {
    agent.logger.debug('shimming https.Server.prototype.emit function')
    shimmer.wrap(modExports && modExports.Server && modExports.Server.prototype,
      'emit',
  ...
  let wrapped = shimmer.wrap(modExports, 'request', httpShared.traceOutgoingRequest(agent, 'https', 'request'))
  ...
  wrapped = shimmer.wrap(modExports, 'get', httpShared.traceOutgoingRequest(agent, 'https', 'get'))
  if (isImportMod && wrapped) {
