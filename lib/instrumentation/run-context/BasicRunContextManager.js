...
class BasicRunContextManager extends AbstractRunContextManager {
  constructor (log, runContextClass) {
    super(log)
    this._runContextClass = runContextClass || RunContext
  ...
  enable () {
    // XXX make this public or add `root()` or `getRoot()` to be used the way OTel API uses `ROOT_CONTEXT` export.
    this._root = new this._runContextClass()
    return this
  }
