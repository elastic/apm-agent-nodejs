...
//
// XXX Update this desc. Also warn this class shouldn't be used directly. Instead use _runCtxMgr._runContextClass? Or ... something public on it?
...
class RunContext {
  ...
  enterTrans (trans) {
    return new this.constructor(trans)
  }
  ...
  enterSpan (span) {
    ...
    newSpans.push(span)
    return new this.constructor(this._trans, newSpans)
  }
  ...
  // examples.)
  exitSpan (span) {
    // XXX need special _deactivateOnEnd differentiation here?
    console.log('XXX exitSpan: span %s %s', span.id, span.name)
    let newRc = null
    let newSpans
    ...
    if (lastSpan && lastSpan.id === span.id) {
      ...
      newRc = new this.constructor(this._trans, newSpans)
    } else {
  ...
  // XXX noop
  getValue (key) {
    console.log('XXX RunContext.getValue(%o)', key)
    return undefined
  }

  setValue (key, value) {
    console.log('XXX RunContext.setValue(%o, %s)', key, value)
    return this
  }

  deleteValue (key) {
    console.log('XXX RunContext.deleteValue(%o)', key)
    return this
  }
