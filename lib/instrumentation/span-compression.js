...
class SpanCompression {
  ...
  _checkDuration (currentMs, additionalMs, type) {
    const totalDurationAsSeconds = (currentMs + additionalMs) / 1000

    if (type === STRATEGY_EXACT_MATCH && (totalDurationAsSeconds < (this._agent._conf.spanCompressionExactMatchMaxDuration * 1000))) {
      return true
    }

    if (type === STRATEGY_SAME_KIND && (totalDurationAsSeconds < (this._agent._conf.spanCompressionSameKindMaxDuration * 1000))) {
      return true
    }
  ...
  tryToCompressRegular (compositeSpan, spanToCompress) {
    ...
    if (this.duration <= (this._agent._conf.spanCompressionSameKindMaxDuration * 1000) && spanToCompress._duration <= (this._agent._conf.spanCompressionSameKindMaxDuration * 1000)) {
      ...
      compositeSpan.name = 'Calls to ' + compositeSpan._destination.service.resource
      return true
    }
