...
class SpanCompression {
  ...
  _getCompressionStrategy (parentSpan, toCompressSpan) {
    if (!this._isEnabled() || !parentSpan._destination || !toCompressSpan._destination) {
      return false
    }

    const isSameKind = parentSpan.type === toCompressSpan.type &&
    ...
    if(!this._checkDuration(parentSpan._duration, toCompressSpan._duration, strategy)) {
      return false
    }
