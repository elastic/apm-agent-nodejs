...
module.exports = function (ins) {
  function wrapFunction (fn) {
    ...
    function instrumented () {
      ...
      ins.currentTransaction = transaction
  ...
  function activator (fn) {
    return function () {
      ...
      if (typeof arguments[index] === 'function') {
        arguments[index] = wrapFunction(arguments[index])
      }
  ...
  wrap(net.Server.prototype, '_listen2', function (original) {
    return function () {
      this.on('connection', function (socket) {
        if (socket._handle) {
          socket._handle.onread = wrapFunction(socket._handle.onread)
        }
      })
      ...
      try {
        ...
        if (this._handle && this._handle.onconnection) {
          this._handle.onconnection = wrapFunction(this._handle.onconnection)
        }
      }
    }
  })
  ...
  wrap(net.Socket.prototype, 'connect', function (original) {
    return function () {
      ...
      if (args[1]) args[1] = wrapFunction(args[1])
      var result = original.apply(this, args)
      if (this._handle) {
        this._handle.onread = wrapFunction(this._handle.onread)
      }
      return result
    }
  })
  ...
  function wrapPromise () {
    ...
    function ensureAslWrapper (promise) {
      if (!promise.__asl_wrapper) {
        promise.__asl_wrapper = wrapFunction(propagateAslWrapper)
      }
    }
