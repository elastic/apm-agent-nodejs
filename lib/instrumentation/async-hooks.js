...
module.exports = function (ins) {
  ...
  function wrapPromise () {
    ...
    if (v6plus) {
      global.Promise = require('./es6-wrapped-promise.js')(Promise, ensureObWrapper)
    } else {
    ...
    function ensureObWrapper (promise, overwrite) {
      if (!promise.__ob_wrapper || overwrite) {
        promise.__ob_wrapper = ins.bindFunction(propagateObWrapper)
      }
    }

    function propagateObWrapper (ctx, fn, result, next) {
      var nextResult
      try {
        ...
        if (nextResult instanceof Promise) {
          next.__ob_wrapper = function proxyWrapper () {
            var obWrapper = nextResult.__ob_wrapper || propagateObWrapper
            return obWrapper.apply(this, arguments)
          }
        } else {
          ensureObWrapper(next, true)
        }
      }
    }
    ...
    function wrapThen (original) {
      return function wrappedThen () {
        ...
        next.__ob_wrapper = function proxyWrapper (ctx, fn, val, last) {
          if (promise.__ob_wrapper) {
            promise.__ob_wrapper(ctx, function () {}, null, next)
            return next.__ob_wrapper(ctx, fn, val, last)
          }
          return propagateObWrapper(ctx, fn, val, last)
        }
        ...
        function bind (fn) {
          ...
          return ins.bindFunction(function (val) {
            var result = (promise.__ob_wrapper || propagateObWrapper)(this, fn, val, next)
            if (result.error) {
