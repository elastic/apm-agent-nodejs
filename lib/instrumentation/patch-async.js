...
module.exports = function (ins) {
    ...
    return function () {
      ...
      try {
        ...
        if (this._handle && this._handle.onconnection) {
          this._handle.onconnection = ins.bindFunction(
  ...
  if (zlib && zlib.Deflate && zlib.Deflate.prototype) {
    ...
    if (proto._transform) {
      ...
      // plain ol' streams
      massWrap(proto, ['write', 'flush', 'end'], activator);
    }
  }
  ...
  if (crypto) {
    ...
    massWrap(crypto, cryptoFunctions, activator);
  }
  ...
  function wrapPromise() {
    ...
    function wrapThen(original) {
      return function wrappedThen() {
        var promise = this;
        var next = original.apply(
          promise,
          Array.prototype.map.call(arguments, bind),
        );
