...
module.exports = function (ins) {
  ...
  function wrapPromise () {
    ...
    function wrappedPromise (executor) {
      ...
      function wrappedExecutor (resolve, reject) {
        ...
        function wrappedResolve (val) {
          ensureElasticAPMWrapper(promise, false)
          return resolve(val)
        }
    ...
    if (v6plus) {
      global.Promise = require('./es6-wrapped-promise.js')(Promise, ensureElasticAPMWrapper)
    } else {
    ...
    function ensureElasticAPMWrapper (promise, overwrite) {
      if (!promise[wrapperSym] || overwrite) {
        promise[wrapperSym] = ins.bindFunction(propagateElasticAPMWrapper)
      }
    }

    function propagateElasticAPMWrapper (ctx, fn, result, next) {
      var nextResult
      try {
        ...
        if (nextResult instanceof Promise) {
          next[wrapperSym] = function proxyWrapper () {
            var elasticAPMWrapper = nextResult[wrapperSym] || propagateElasticAPMWrapper
            return elasticAPMWrapper.apply(this, arguments)
          }
        } else {
          ensureElasticAPMWrapper(next, true)
        }
      }
    }
    ...
    function wrapThen (original) {
      return function wrappedThen () {
        ...
        next[wrapperSym] = function proxyWrapper (ctx, fn, val, last) {
          if (promise[wrapperSym]) {
            promise[wrapperSym](ctx, function () {}, null, next)
            return next[wrapperSym](ctx, fn, val, last)
          }
          return propagateElasticAPMWrapper(ctx, fn, val, last)
        }
        ...
        function bind (fn) {
          ...
          return ins.bindFunction(function (val) {
            var result = (promise[wrapperSym] || propagateElasticAPMWrapper)(this, fn, val, next)
            if (result.error) {
