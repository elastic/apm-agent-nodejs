...
const massWrap = shimmer.massWrap

const v7plus = semver.gte(process.version, '7.0.0')
const v11plus = semver.gte(process.version, '11.0.0')
...
module.exports = function (ins) {
  ...
  const patchGlobalTimers = global.setTimeout === timers.setTimeout
  ...
  if (crypto) {
    const cryptoFunctions = ['pbkdf2', 'randomBytes']
  ...
  let instrumentPromise = isNative(global.Promise)
  ...
  function wrapPromise () {
    ...
    function wrapThen (original) {
      return function wrappedThen () {
        const promise = this
        const next = original.apply(promise, Array.prototype.map.call(arguments, bind))
  ...
  function activator (fn) {
    ...
    const fallback = function () {
      let args
      const cbIdx = arguments.length - 1
