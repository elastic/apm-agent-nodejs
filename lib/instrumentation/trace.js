...
function Trace (transaction) {
  ...
  debug('init trace', { uuid: this.transaction._uuid })
}
...
Trace.prototype.start = function (signature, type) {
  ...
  debug('start trace', { uuid: this.transaction._uuid, signature: signature, type: type })
}
...
Trace.prototype.end = function () {
  ...
  this.ended = true
  debug('ended trace', { uuid: this.transaction._uuid, signature: this.signature, type: this.type })
  this.transaction._recordEndedTrace(this)
}

Trace.prototype.duration = function () {
  if (!this.ended) {
    debug('Trying to call trace.duration() on un-ended Trace', { uuid: this.transaction._uuid })
    return null
  }
...
Trace.prototype.startTime = function () {
  if (!this.ended || !this.transaction.ended) {
    debug('Trying to call trace.startTime() for un-ended Trace/Transaction', { uuid: this.transaction._uuid })
    return null
  }
...
Trace.prototype.ancestors = function () {
  if (!this.ended || !this.transaction.ended) {
    debug('Trying to call trace.ancestors() for un-ended Trace/Transaction', { uuid: this.transaction._uuid })
    return null
  }
