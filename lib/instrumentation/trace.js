...
Trace.prototype.truncate = function () {
  if (!this.started) {
    debug('tried to truncate non-started trace - ignoring %o', { uuid: this.transaction._uuid, signature: this.signature, type: this.type })
    return
  } else if (this.ended) {
    debug('tried to truncate already ended trace - ignoring %o', { uuid: this.transaction._uuid, signature: this.signature, type: this.type })
    return
  }
  this.truncated = true
  this.end()
...
Trace.prototype.end = function () {
  ...
  this.ended = true
  debug('ended trace %o', { uuid: this.transaction._uuid, signature: this.signature, type: this.type, truncated: this.truncated })
