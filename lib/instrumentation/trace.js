...
Trace.prototype.startTime = function () {
  if (!this.ended || !this.transaction.ended) {
    debug('tried to call trace.startTime() for un-ended trace/transaction %o', {id: this.transaction.id, name: this.name, type: this.type})
    return null
  }
...
Trace.prototype.ancestors = function () {
  if (!this.ended || !this.transaction.ended) {
    debug('tried to call trace.ancestors() for un-ended trace/transaction %o', {id: this.transaction.id, name: this.name, type: this.type})
    return null
  }
