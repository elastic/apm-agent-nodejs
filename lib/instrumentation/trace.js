...
function Trace (transaction) {
  ...
  logger.trace('[%s] init trace', this.transaction._uuid)
}
...
Trace.prototype.start = function (signature, type) {
  ...
  logger.trace('[%s] start trace (signature: %s, type: %s)', this.transaction._uuid, signature, type)
}
...
Trace.prototype.end = function () {
  ...
  this.ended = true
  logger.trace('[%s] ended trace (signature: %s, type: %s)', this.transaction._uuid, this.signature, this.type)
  this.transaction._recordEndedTrace(this)
}

Trace.prototype.duration = function () {
  if (!this.ended) {
    logger.error('[%s] Trying to call trace.duration() on un-ended Trace!', this.transaction._uuid)
    return null
  }
...
Trace.prototype.startTime = function () {
  if (!this.ended || !this.transaction.ended) {
    logger.error('[%s] Trying to call trace.startTime() for un-ended Trace/Transaction!', this.transaction._uuid)
    return null
  }
...
Trace.prototype.ancestors = function () {
  if (!this.ended || !this.transaction.ended) {
    logger.error('[%s] Trying to call trace.ancestors() for un-ended Trace/Transaction!', this.transaction._uuid)
    return null
  }
