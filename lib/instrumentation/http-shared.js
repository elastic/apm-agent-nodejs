...
const { transaction } = require('elastic-apm-http-client/lib/truncate')

function transactionStartedByOtelBridge(agent) {
  console.log("skipping normal transaction creation")
  return true
}
    ...
    return function (event, req, res) {
      if (event === 'request') {
        ...
        if (shouldIgnoreRequest(agent, req)) {
          ...
          if(agent._oTelEntrySpan) {
            trans._oTelSpan = agent._oTelEntrySpan
          }
        ...
        // clear oTel entry span
        agent._oTelEntrySpan = null
      }
