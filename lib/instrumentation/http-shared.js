...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (...args) {
      ...
      const setter = function (carrier, name, value) {
        if (!value) {
          return
        }
        carrier[name] = value

        if (agent._conf.useElasticTraceparentHeader && name === 'traceparent') {
          carrier['elastic-apm-traceparent'] = value
        }
      }
      const newHeaders = Object.assign({}, options.headers)
      if (parent && parent._context && span) {
        parent.propagateTraceContextHeaders(newHeaders, setter)
      } else if (parent && parent._context && !span) {
        // headers still need to propagate
        propagateTraceContextHeaders(newHeaders, parent._context, setter)
      }
