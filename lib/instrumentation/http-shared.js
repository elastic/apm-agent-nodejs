    ...
    return function (event, req, res) {
      if (event === 'request') {
        ...
        if (isRequestBlacklisted(agent, req)) {
          ...
          // XXX Is this still needed? What test case catches this? Otherwise I'd like to drop this API and call.
...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (...args) {
      ...
      // XXX Consider binding the callback, if any, to the *parent* context
      //     before the startSpan, and add tests for that.
      ...
      // XXX Consider binding this to the *parent* context before the startSpan
      ...
      function onresponse (res) {
        // XXX Behaviour change / Reviewer notes
        //    With the old async-hooks.js-based context management, with Node.js
        //    v12.0 - 12.2, when this onresponse ran, the currentTransaction was
        //    null, because of this core Node async_hooks bug:
        //      https://github.com/nodejs/node/pull/27477
        //    where an "init" async hook would not be called for a re-used HTTPParser.
        //    This was worked around in the agent in #1339 with this change:
        //        ins._recoverTransaction(span.transaction)
        //    which just sets `ins.currentTransaction = span.transaction`.
        //    Before the workaround `node test/instrumentation/modules/http/outgoing.test.js`
        //    would fail:
        //         not ok 19 test exited without ending: http.request(options, callback)
        //         ---
        //           operator: fail
        //           at: process.<anonymous> (.../node_modules/tape/index.js:94:23)
        //           stack: |-
        //             Error: test exited without ending: http.request(options, callback)
        //
        //    A possibly better workaround would have been to use ins.bindFunction
        //    to bind this `onresponse` to the current span and transaction.
        //
        //    With the new AsyncHooksRunContextManager, neither of these are
        //    necessary. The `init` async hook *is* still missed in node v12.0-12.2,
        //    but this `onresponse()` runs within the context of this trans and
        //    span in all outgoing.test.js and in manual tests. I.e. adding:
        //      assert(ins.currSpan() === span)
        //    here, still passes the test suite.
