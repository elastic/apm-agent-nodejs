  ...
  return function (orig) {
    return function (...args) {
      ...
      endOfStream(req, function () {
        // End span only when request is aborted as EOS fires for
        // 'abort', 'close', 'end' events which might end span prematurely
        if (req.aborted) {
          if (span.ended) return
          agent.logger.debug('unexpected end of outgoing http request %o', { id: id })
