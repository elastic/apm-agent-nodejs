...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (...args) {
      // XXX See if can avoid instrumenting request to APM server... would
      //     be nice if could be more explicit from the APM server client code
      //     itself. Special header? Use uninstrumented endpoint (would need
      ...
      // console.warn('XXX curr span (after startSpan):', agent._instrumentation.currentSpan?.name) // considers 'bindingSpan'
