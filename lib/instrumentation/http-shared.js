  ...
  return function (orig) {
    return function (...args) {
      ...
      if (parent && parent._context && shouldPropagateTraceContext(options)) {
        ...
        const baggageString = parent._context.toBaggageString() // don't like this name
        ...
        if (baggageString) {
          options.headers.baggage = baggageString
        }
      }

      var req = orig.apply(this, newArgs)
      console.warn('XXX outgoing instrumented request:', req.method, req.path, options.headers)
      if (!span) return req
