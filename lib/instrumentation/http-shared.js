 ...
 * -- made "safe" to use as the `options` argument to `http.request` and
 * `https.request`.
 *
 * By "safe" here we mean that it will not accidentally be considered to be
 * a `url` argument. This matters in the instrumentation below because the
 * following are handled differently:
 *      http.request(<options>, <bogus-callback>)
 *      http.request(<url>, <bogus-callback>)
 */
...
if (!urlToHttpOptions) {
  ...
  safeUrlToHttpOptions = function (url) {
    ...
    // Delete fields so `isURL(options) === false` where `isURL()` is from
    // "node/lib/internal/url.js". Specically we are dropping the
    // `Symbol(context)` field.
