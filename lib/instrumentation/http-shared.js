    ...
    return function (event, req, res) {
      if (event === 'request') {
        ...
        if (isRequestBlacklisted(agent, req)) {
          ...
          res[symbols.requestTransaction] = trans
...
var writeHeadIsWrapped = false
exports.instrumentWriteHead = function (agent, ServerResponse) {
  if (writeHeadIsWrapped) return
  if (!ServerResponse) return

  writeHeadIsWrapped = true
  agent.logger.debug('shimming ServerResponse.prototype.writeHead function')
  shimmer.wrap(ServerResponse.prototype, 'writeHead', original => {
    return function wrappedWriteHead () {
      ...
      var trans = this[symbols.requestTransaction]
      this[symbols.requestTransaction] = null
