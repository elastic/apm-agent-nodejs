...
//
// This function recontructs a URL using the request object's
// properties where it can, and falling back to the options
// where it can not.
//
// NOTE: The options argument may not be the same options that are passed to
// http.request if the caller uses the the (url,options,...) method signature.
// The agent normalizes the url and options into a single options array. This
// function expects those pre-normalized options.
//
// @param {req} ClientRequest
// @param {options} object
// @return string
function getUrlFromRequestAndOptions (req, options) {
  if (!req) {
    return false
  }
  options = options || {}
  ...
  const host = req.host || options.hostname || options.host || 'unknown'
  const protocol = req.protocol || req.agent.protocol
