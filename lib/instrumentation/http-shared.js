...
function getInternalIgnoreHeader (res) {
  return res.getHeader ? res.getHeader('x-elastic-apm-agent-nodejs-ignore') : res._headers.host
}
  ...
  return function (orig) {
    return function (...args) {
      ...
      if (getSafeHost(req) === agent._conf.serverHost) {
        ...
        return req
      } else if (getInternalIgnoreHeader(req) === '1') {
        agent.logger.debug('ignore %s request with ignore header flag %o', moduleName, { id: id })
        return req
      } else {
