...
const { getHTTPDestination } = require('./context')
        ...
        if (shouldIgnoreRequest(agent, req)) {
          ...
          endOfStream(res, function (err) {
            ...
            if (agent._conf.errorOnAbortedRequests) {
              const duration = trans._timer.elapsed()
...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (input, options, cb) {
      ...
      const span = ins.createSpan(null, 'external', 'http', { exitSpan: true })
      ...
      const req = ins.withRunContext(spanRunContext, orig, this, ...newArgs)
