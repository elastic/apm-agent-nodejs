...
const { propagateTraceContextHeaders } = require('../tracecontext')
...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (...args) {
      ...
      if (parent && parent._context && span) {
        options.headers = span.propagateTraceContextHeaders(options.headers, parent._context)
      } else if (parent && parent._context && !span) {
        // headers still need to propagate
        options.headers = propagateTraceContextHeaders(options.headers, parent._context)
      }

      if (agent._conf.useElasticTraceparentHeader && options.headers.traceparent) {
        options.headers['elastic-apm-traceparent'] = options.headers.traceparent
      }
