...
exports.traceOutgoingRequest = function (agent, moduleName, method) {
  ...
  return function (orig) {
    return function (...args) {
      ...
      agent.logger.debug({ id }, 'intercepted call to %s.%s', moduleName, method)
      ...
      if (getSafeHost(req) === agent._conf.serverHost) {
        agent.logger.debug({ id }, 'ignore %s request to intake API', moduleName)
        return req
      } else {
        var protocol = req.agent && req.agent.protocol
        agent.logger.debug({ protocol, host: getSafeHost(req), id }, 'request details')
      }
      ...
      function onresponse (res) {
        ...
        agent.logger.debug({ id }, 'intercepted http.ClientRequest response event')
        ins.bindEmitter(res)

        res.prependListener('end', onEnd)

        function onEnd () {
          agent.logger.debug({ id }, 'intercepted http.IncomingMessage end event')
          span.end()
        }
      }
    }
