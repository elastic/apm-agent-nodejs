...
Instrumentation.prototype.add = function (transaction) {
  if (this._client.active && this._client._ff_instrument) {
    this._client.logger.trace('pushing transaction to queue:', transaction.name)
    this._queue.push(transaction)
  } else {
    this._client.logger.trace('ignoring transaction:', transaction.name)
  }
...
Instrumentation.prototype.startTransaction = function (name, type, result) {
  this._client.logger.trace('starting transaction (name: %s, type: %s, result: %s)', name, type, result)
