...
// XXX Deprecated? or call startAndEnterSpan?
...
// XXX new hotness: allows instrmentations to create spans but not bleed
// that current span out to caller if the startSpan is in the same xid. E.g.
// as with s3.js (and I think with @elastic/elasticsearch.js).
Instrumentation.prototype.createSpan = function (name, type, subtype, action, opts) {
  ...
  return tx.createSpan.apply(tx, arguments)
}
...
Instrumentation.prototype.bindFunction = function (fn) {
  assert(!(fn instanceof RunContext),
    'XXX did you mean to call ins.bindFunctiontoRunContext(rc, fn) instead?')
  if (!this._started) {
    return fn
  }
  return this._runCtxMgr.bindFunction(this._runCtxMgr.active(), fn)
}

// XXX Doc this
// XXX should this be "ToRootRunContext" rather than "Empty"? I think yes.
Instrumentation.prototype.bindFunctionToEmptyRunContext = function (fn) {
  if (!this._started) {
    return fn
  }
  return this._runCtxMgr.bindFunction(new RunContext(), fn)
}
...
Instrumentation.prototype.bindFunctionToRunContext = function (runContext, fn) {
  ...
  return this._runCtxMgr.bindFunction(runContext, fn)
}
