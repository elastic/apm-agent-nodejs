...
function Instrumentation (agent) {
  ...
  this._flushingInflightEvents = null
  this._started = false
...
// XXX splain semantics
Instrumentation.prototype.flush = function (cb, timeoutMs) {
  ...
  // process.nextTick(cb); return // XXX no-op

  // XXX explain the semantics from not force draining here.
  // If there is an earlier Instrumentation#flush() waiting to drain, then
  // force drain it now.
  // XXX HERE how to move inflights back to the newer one *and* have them be drained from this one?
  //          Need some "child" handling? Is there a real use case?
  // if (this._flushingInflightEvents) {
  //   this._flushingInflightEvents.drain()
  //   this._flushingInflightEvents = null
  // }

  // If there are no inflight events (likely a common case), then avoid creating
  // additional objects.
  ...
  // Otherwise, there are inflight events to wait for.
  this._flushingInflightEvents = this._inflightEvents
  this._inflightEvents = new InflightEventSet()
  this._flushingInflightEvents.setDrainHandler(cb, timeoutMs)
}
