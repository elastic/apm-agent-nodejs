...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  ...
  if (transaction._bufferedSpanForCompression) {
    this.encodeAndSendSpan(transaction._bufferedSpanForCompression)
  }
...
Instrumentation.prototype.addEndedSpan = function (span) {
  ...
  // if I have ended and I have something buffered, send that buffered thing
  if (span._bufferedSpanForCompression) {
    this.encodeAndSendSpan(span._bufferedSpanForCompression)
  }
  ...
  else if (!span._parentSpan._bufferedSpanForCompression) {
    // span is compressible and there's nothing buffered
    // add to buffer, move on
    span._parentSpan._bufferedSpanForCompression = span
  } else if (!span._parentSpan._bufferedSpanForCompression.tryToCompress(span)) {
    // we could not compress span so SEND bufferend span
    // and buffer the span we could not compress
    this.encodeAndSendSpan(span._parentSpan._bufferedSpanForCompression)
    span._parentSpan._bufferedSpanForCompression = span
  }
