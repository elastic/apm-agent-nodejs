...
Instrumentation.prototype._startHook = function () {
  ...
  const IITM_MODULES = {
    ...
    // This smithy-client entry isn't used for `@aws-sdk/client-s3` ESM support
    // because the smithy-client is transitively `require`d by CommonJS aws-sdk
    // code.
    '@aws-sdk/smithy-client': { instrumentImportMod: false },
    fastify: {
      // Fastify instrumentation changes the top-level export, so needs to
      // work with the module.
      instrumentImportMod: true
    }
  }
  this._iitmHook = IitmHook(Object.keys(IITM_MODULES), function (modExports, modName, modBaseDir) {
    assert(modBaseDir, `XXX Why is there no modBaseDir? modName=${modName}`)
    ...
    if (IITM_MODULES[modName].instrumentImportMod) {
      ...
      modExports.default = self._patchModule(modExports.default, modName, version, enabled, false)
      return modExports
    }
  })
}
 ...
 *    by any type. For an `import` this is a `Module` object.
 * @param {string} name
 ...
 */
Instrumentation.prototype._patchModule = function (modExports, name, version, enabled, isImportMod) {
  this._agent.logger.debug('instrumenting %s@%s module (enabled=%s, isImportMod=%s)', name, version, enabled, isImportMod)
