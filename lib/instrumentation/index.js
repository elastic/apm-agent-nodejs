...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  ...
  if (this._started) {
    ...
    if (!payload) return agent.logger.debug({ trans: transaction.id, trace: transaction.traceId }, 'transaction ignored by filter')
    agent.logger.debug({ trans: transaction.id, trace: transaction.traceId }, 'sending transaction')
    agent._transport.sendTransaction(payload)
  } else {
    agent.logger.debug({ trans: transaction.id, trace: transaction.traceId }, 'ignoring transaction')
  }
}
...
Instrumentation.prototype.addEndedSpan = function (span) {
  ...
  if (this._started) {
    agent.logger.debug({ span: span.id, parent: span.parentId, trace: span.traceId, name: span.name, type: span.type }, 'encoding span')
    span._encode(function (err, payload) {
      if (err) {
        agent.logger.error({ span: span.id, parent: span.parentId, trace: span.traceId, name: span.name, type: span.type, error: err.message }, 'error encoding span')
        return
      }
      ...
      if (!payload) {
        agent.logger.debug({ span: span.id, parent: span.parentId, trace: span.traceId, name: span.name, type: span.type }, 'span ignored by filter')
        return
      }

      agent.logger.debug({ span: span.id, parent: span.parentId, trace: span.traceId, name: span.name, type: span.type }, 'sending span')
      if (agent._transport) agent._transport.sendSpan(payload)
    })
  } else {
    agent.logger.debug({ span: span.id, parent: span.parentId, trace: span.traceId, name: span.name, type: span.type }, 'ignoring span')
  }
}
...
Instrumentation.prototype._recoverTransaction = function (trans) {
  ...
  this._agent.logger.debug({
    wrong: this.currentTransaction ? this.currentTransaction.id : this.currentTransaction,
    correct: trans.id,
    trace: trans.traceId
