...
var Instrumentation = module.exports = function (agent) {
  ...
  if (!this._agent.instrument || !this._agent.active) return
...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  if (this._agent.active && this._agent.instrument) {
...
Instrumentation.prototype.endTransaction = function () {
  ...
  if (!trace) return this._agent.logger.warn('cannot end transaction - no active trace found')
  trace.transaction.end()
}

Instrumentation.prototype.setTransactionName = function (name) {
  var trace = this.currentTrace
  if (!trace) return this._agent.logger.warn('no active trace found - cannot set transaction name')
