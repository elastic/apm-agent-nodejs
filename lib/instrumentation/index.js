  ...
  this._hook = hook(this._patches.keys, function (exports, name, basedir) {
    ...
    if (basedir) {
      ...
      try {
        ...
        self._agent.logger.debug({ err }, 'could not shim %s module', name)
        return exports
      }
    } else {
...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  ...
  if (this._started) {
    ...
    if (!payload) return agent.logger.debug({ span: transaction }, 'transaction ignored by filter')
    agent.logger.debug({ span: transaction }, 'sending transaction')
    agent._transport.sendTransaction(payload)
  } else {
    agent.logger.debug({ span: transaction }, 'ignoring transaction')
  }
}
...
Instrumentation.prototype.addEndedSpan = function (span) {
  ...
  if (this._started) {
    agent.logger.debug({ span }, 'encoding span')
    span._encode(function (err, payload) {
      if (err) {
        agent.logger.error({ span, err }, 'error encoding span')
        return
      }
      ...
      if (!payload) {
        agent.logger.debug({ span }, 'span ignored by filter')
        return
      }

      agent.logger.debug({ span }, 'sending span')
      if (agent._transport) agent._transport.sendSpan(payload)
    })
  } else {
    agent.logger.debug({ span }, 'ignoring span')
  }
}
