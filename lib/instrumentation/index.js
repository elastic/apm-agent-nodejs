...
function groupTransactions (transactions) {
  ...
  return Object.keys(groups).map(function (key) {
    ...
    return {
      ...
      result: trans.result,
      kind: trans.type,
      timestamp: groupingTs(trans).toISOString(),
      durations: durations
    }
  })
}
...
function groupTraces (traces) {
  ...
  return Object.keys(groups).map(function (key) {
    ...
    return {
      ...
      kind: trace.type,
      timestamp: groupingTs(trace).toISOString(),
      frames: [], // TODO
...
function traceGroupingKey (trace) {
  return groupingTs(trace._start).getTime() + '|' + trace.transaction.name + '|' + trace.signature + '|' + trace.type
}
