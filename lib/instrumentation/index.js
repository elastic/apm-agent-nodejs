...
Instrumentation.prototype._startHook = function () {
  ...
  this._hook = new RitmHook(this._patches.keys, function (exports, name, basedir) {
  ...
  // XXX
  const IITM_MODULES = ['ioredis', 'knex', 'pg']
  this._iitmHook = IitmHook(IITM_MODULES, function (modExports, modName, modBaseDir) {
    const enabled = self._isModuleEnabled(modName)
    if (!enabled) { // XXX consider this for above as well? Could be separate change and removal of guard from all instrs.
      return
    }
    console.log('XXX IITMHook modName="%s" (modBaseDir=%s)', modName, modBaseDir)
    if (!modBaseDir) {
      console.log('XXX no modBaseDir? What should we do here? What are examples?', modName, modBaseDir)
    }

    const version = self._getPackageVersion(modName, modBaseDir)
    // XXX handle there *not* being a loadable version, some instrs might support regardless (e.g. core modules)
    // XXX this could be where we do the semver checks for supported versions with a richer MODULES. Would that help for our myriad special guard checking?
    if (modExports && modExports.default) {
      console.log('XXX doing the modExports.default thing for module %s', modName)
      modExports.default = self._patchModule(modExports.default, modName, version, enabled)
      return modExports
    } else {
      return self._patchModule(modExports, modName, version, enabled)
    }
  })
}
...
Instrumentation.prototype._getPackageVersion = function (modName, modBaseDir) {
  try {
    const version = JSON.parse(fs.readFileSync(path.join(modBaseDir, 'package.json'))).version
    ...
    this._agent.logger.debug({ modName, modBaseDir, err }, 'could not load package version')
    return undefined
  }
}
