...
Instrumentation.prototype.start = function () {
  ...
  const lambdaHandlerInfo = this.getLambdaHandlerInfo(process.env)
  if (lambdaHandlerInfo) {
    this.addPatch(
      lambdaHandlerInfo.filePath,
      require('./modules/_lambda-handler')
    )
  }
...
Instrumentation.prototype._startHook = function () {
  ...
  this._hook = hook(this._patches.keys, function (exports, name, basedir) {
    ...
    const lambdaHandlerInfo = self.getLambdaHandlerInfo(process.env)
    const isHandlingLambda = lambdaHandlerInfo && lambdaHandlerInfo.module === name
...
Instrumentation.prototype._patchModule = function (exports, name, version, enabled) {
  ...
  } else if (name === lambdaHandlerInfo.module) {
    patches = this._patches.get(lambdaHandlerInfo.filePath)
  }
...
Instrumentation.prototype.getLambdaHandlerInfo = function (env) {
  ...
  for (let instrumentedModule of MODULES) {
    if (Array.isArray(instrumentedModule)) {
      instrumentedModule = instrumentedModule[0]
    }
    if (handlerModule === instrumentedModule) {
      this._log.info(
        'Unable to instrument Lambda handler due to conflict with %s, please choose a different Lambda handler name',
        instrumentedModule
      )
      return
    }
