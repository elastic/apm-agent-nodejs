...
Instrumentation.prototype.start = function () {
  ...
  shimmer({ logger: logger.error })
  require('./async-hooks')(this._agent)

  logger.trace('shimming function Module._load')
  ...
  shimmer.wrap(Module, '_load', function (orig) {
    return function (file) {
      return modules.patch(file, orig.apply(this, arguments), self._agent)
    }
  })
}
...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  if (this._agent.instrument) {
    logger.trace('[%s] pushing transaction to queue', transaction._uuid)
    this._queue.push(transaction)
  } else {
    logger.trace('[%s] ignoring transaction', transaction._uuid)
  }
...
Instrumentation.prototype.endTransaction = function () {
  ...
  if (!trace) return logger.warn('cannot end transaction - no active trace found')
  trace.transaction.end()
}

Instrumentation.prototype.setTransactionName = function (name) {
  var trace = this.currentTrace
  if (!trace) return logger.warn('no active trace found - cannot set transaction name')
...
Instrumentation.prototype._send = function () {
  logger.trace('flushing transaction queue')
...
Instrumentation.prototype._queueSend = function () {
  logger.trace('setting timer to flush transaction queue')
