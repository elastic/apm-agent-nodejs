...
Instrumentation.prototype.add = function (transaction) {
  if (this._client.active && this._client._ff_instrument) {
    this._client.logger.trace('[%s] pushing transaction to queue:', transaction._uuid, transaction.name)
    this._queue.push(transaction)
  } else {
    this._client.logger.trace('[%s] ignoring transaction:', transaction._uuid, transaction.name)
  }
