...
var MODULE_PATCHERS = [
  ...
  { modPath: '@redis/client/dist/lib/client/index.js' },
...
Instrumentation.prototype.start = function (runContextClass) {
  ...
  // config.addPatch.
  for (let i of MODULE_PATCHERS) {
    let patcher;
    if (i.patcher) {
      patcher = path.resolve(__dirname, i.patcher);
    } else {
      ...
      patcher = path.resolve(
        __dirname,
        'modules',
        i.modPath + (i.modPath.endsWith('.js') ? '' : '.js'),
      );
    }

    this._patcherReg.add(i.modPath, patcher);
  }
  ...
  this._ritmHook = new RitmHook(
    ...
    function (exports, modPath, basedir) {
      ...
      if (version !== undefined) {
        ...
        try {
          // TODO: cache this, share with _getPackageVersion below.
          version = JSON.parse(fs.readFileSync(pkg)).version;
        } catch (e) {
          self._log.debug('could not patch %s module: %s', modPath, e.message);
          return exports;
        }
      }
