...
Instrumentation.prototype.setBaggageEntry = function (key, entry) {
  if (!this.currentTransaction) {
    this._agent.logger.debug('no active transaction found - cannot set baggage entry')
    return
  }
  this.currentTransaction.setBaggageEntry(key, entry)
}
Instrumentation.prototype.getBaggageEntry = function (key) {
  if (!this.currentTransaction) {
    this._agent.logger.debug('no active transaction found - cannot get baggage entry')
    return
  }
  return this.currentTransaction.getBaggageEntry(key)
}
