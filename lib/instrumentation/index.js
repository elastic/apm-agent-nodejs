...
Instrumentation.prototype._maybeLoadLambdaPatcher = function () {
  ...
  if (lambdaHandlerInfo) {
    ...
    console.log('XXX add lambda info to _patcherReg');
...
Instrumentation.prototype._restartHooks = function () {
  ...
  console.log('XXX ritmModulesArg: ', this._patcherReg.ritmModulesArg());
  this._ritmHook = new RitmHook(
    this._patcherReg.ritmModulesArg(),
    { internals: true },
    function (exports, modPath, basedir) {
      console.log('XXX RitmHook: modPath: ', modPath);
  ...
  if (process.env.TRENT_DEBUG === 'yes') {
    // iitmHookArg.push('/var/task/index.js'); // XXX
    // IITM_MODULES['/var/task/index.js'] = { instrumentImportMod: false };
    IITM_MODULES['/var/task/index.js'] = { instrumentImportMod: true };
    IITM_MODULES['/var/task/index.mjs'] = { instrumentImportMod: true };
  }
  const iitmHookArg = Object.keys(IITM_MODULES);
  console.log('XXX iitmHookArg: ', iitmHookArg);
  this._iitmHook = IitmHook(
    // TODO: Eventually derive this from `_patcherRegistry`.
    iitmHookArg,
    function (modExports, modName, modBaseDir) {
      console.log('XXX IitmHook: modName: ', modName);
