...
Instrumentation.prototype.start = function (runContextClass) {
  ...
  if (
    ...
    // XXX How to watch/maintain this? Can bitrot.js watch for this? Is there harm in just always turning this on? E.g. try with node v20.
    semver.satisfies(process.version, '>=14 <19')
  ) {
    this._log.debug('instrumenting azure-functions')
    // setInterval(() => {
    //   this._log.flush() // XXX not sure this is doing anything
    // }, 10 * 1000)
    azureFunctionsInstr.instrument(this._agent)
  } else {
    this._log.info('XXX NOT instr azure-functions', this._isModuleEnabled('azure-functions'), process.env.FUNCTIONS_WORKER_RUNTIME, semver.satisfies(process.version, '>=14 <19'))
  }
}
...
Instrumentation.prototype._patchModule = function (exports, name, version, enabled) {
  ...
  if (patches) {
    for (let patch of patches) {
      ...
      exports = patch(exports, this._agent, { name, version, enabled })
    }
  }
  return exports
}
