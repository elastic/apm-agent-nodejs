...
Instrumentation.prototype.addPatch = function (name, handler) {
  if (typeof handler !== 'function' && typeof handler !== 'string') {
    this._agent.logger.error('Invalid patch handler type')
    return
  }

  return this._patches.add(name, handler)
}

Instrumentation.prototype.removePatch = function (name, handler) {
  return this._patches.delete(name, handler)
}

Instrumentation.prototype.clearPatches = function (name) {
  return this._patches.clear(name)
}
