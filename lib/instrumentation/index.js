...
Instrumentation.prototype._patchModule = function (exports, name, version, enabled) {
  ...
  if (patches) {
    for (let patch of patches) {
      ...
      // XXX This doesn't catch, and should. If there are multiple patches for a
      //    given module, then a crash in an early one will break usage of the
      //    subsequent ones. Also, I'm not sure of the impact on the `hook()`
      //    call. The failure here is also silent, so hard to notice.
