...
const TRANS_TYPE_FROM_TRIGGER_TYPE = {
  ...
  [TRIGGER_TIMER]: 'timer' // XXX unspec'd. Is there any prior art?
}
...
function instrument (agent) {
  ...
  // // XXX `appStart` theoretically could be useful to do a grab of central config, get APM server version, etc.,
  // //    but I don't think we'll bother for now.
  // d = core.registerHook('appStart', (hookCtx) => { console.log('XXX appStart') })
  // hookDisposables.push(d)

  // XXX Use appTerminate with an async callback to only resolve after a fastish flush -- to ensure we flush last events.
  //     I don't think we can simulate this to test it.
  d = core.registerHook('appTerminate', (hookCtx) => {
