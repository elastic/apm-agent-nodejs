...
Span.prototype.end = function (endTime) {
  ...
  if (this._capturedStackTrace !== null && this.shouldSetSpanStackTrace()) {
    ...
    var self = this
    stackman.callsites(this._capturedStackTrace, function (err, callsites) {
      if (err) return self._stackObj.reject(err)
      if (!TEST) callsites = callsites.filter(filterCallsite)

      var next = afterAll((err, res) => {
        err ? self._stackObj.reject(err) : self._stackObj.resolve(res)
      })
...
Span.prototype.shouldSetSpanStackTrace = function () {
  if (this._agent._conf.spanFramesMinDuration === -1) {
    return true
  }

  if (this._agent._conf.spanFramesMinDuration === 0) {
    return false
  }

  return this.duration() / 1000 > this._agent._conf.spanFramesMinDuration
}
