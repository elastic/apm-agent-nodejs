...
function Span (transaction, name, ...args) {
  ...
  this._agent.logger.debug({ span: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action }, 'start span')
}
...
Span.prototype.customStackTrace = function (stackObj) {
  this._agent.logger.debug({ span: this.id, parent: this.parentId, trace: this.traceId }, 'applying custom stack trace to span')
  this._recordStackTrace(stackObj)
}

Span.prototype.end = function (endTime) {
  if (this.ended) {
    this._agent.logger.debug({ span: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action }, 'tried to call span.end() on already ended span')
    return
  }
  ...
  this.ended = true
  this._agent.logger.debug({ span: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action }, 'ended span')
...
Span.prototype._encode = function (cb) {
  ...
  function done (err, frames) {
    if (err) {
      self._agent.logger.debug({ span: self.id, parent: self.parentId, trace: self.traceId, name: self.name, type: self.type, subtype: self.subtype, action: self.action, err: err.message }, 'could not capture stack trace for span')
    }
