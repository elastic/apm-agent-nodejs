...
Span.prototype.end = function (endTime, outcome = false) {
  ...
  this._setOutcomeFromSpanEnd(outcome)
...
Span.prototype._isValidOutcome = function (outcome) {
  return outcome === constants.SPAN.OUTCOME.FAILURE ||
    outcome === constants.SPAN.OUTCOME.SUCCESS ||
    outcome === constants.SPAN.OUTCOME.UNKNOWN
}
...
Span.prototype._setOutcomeFromErrorCapture = function (outcome) {
  ...
  // honor its value
  if (this.isOutcomeFrozen) {
    return
  }
  this.outcome = outcome
}

Span.prototype._setOutcomeFromStatusCode = function (statusCode) {
  // if an outcome's been set from the API we
  // honor its value
  if (this.isOutcomeFrozen) {
    return
...
Span.prototype._setOutcomeFromSpanEnd = function (outcome) {
  // if an outcome's been set from the API we
  // honor its value
  if (this.isOutcomeFrozen) {
    return
  ...
  // if the end-user-programmer passed us a valid outcome, use it
  // otherwise, if the span outcome is still unset and not frozen,
  // consider an ended span a success
  if (this._isValidOutcome(outcome)) {
    this.outcome = outcome
  } else if (this.outcome === constants.SPAN.OUTCOME.UNKNOWN && !this.isOutcomeFrozen) {
    this.outcome = constants.SPAN.OUTCOME.SUCCESS
  }
}
