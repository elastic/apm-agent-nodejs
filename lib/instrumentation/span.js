...
function Span (transaction, name, ...args) {
  ...
  this._agent.logger.debug({ span: this }, 'start span')
}
...
Span.prototype.customStackTrace = function (stackObj) {
  this._agent.logger.debug({ span: this }, 'applying custom stack trace to span')
  this._recordStackTrace(stackObj)
}

Span.prototype.end = function (endTime) {
  if (this.ended) {
    this._agent.logger.debug({ span: this }, 'tried to call span.end() on already ended span')
    return
  }
  ...
  this.ended = true
  this._agent.logger.debug({ span: this }, 'ended span')
...
Span.prototype._encode = function (cb) {
  ...
  function done (err, frames) {
    if (err) {
      self._agent.logger.debug({ span: self, err }, 'could not capture stack trace for span')
    }
