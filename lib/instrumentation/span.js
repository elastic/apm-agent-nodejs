...
Span.prototype._encode = function (cb) {
  ...
  if (self.isComposite()) {
    ...
    payload.duration = self._compression.duration / 1000
  }

  if (self.isCompositeSameKind()) {
    // rename
    let callsTo = self && self._destination && self._destination.service && self._destination.service.resource
    if (!callsTo) {
      callsTo = 'unknown'
    }
    payload.name = `Calls to ${callsTo}`
  }
