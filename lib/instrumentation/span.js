...
Span.prototype.end = function (endTime) {
  ...
  if (this.ended) {
    agent.logger.debug('tried to call span.end() on already ended span %o', { span: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action })
    return
  }
  ...
  this.ended = true
  agent.logger.debug('ended span %o', { span: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action })
  ...
  if (this._capturedStackTrace !== null &&
      ...
      this.duration() / 1000 > agent._conf.spanFramesMinDuration) {
    ...
    gatherStackTrace(
      ...
      agent._conf.sourceLinesSpanAppFrames,
      agent._conf.sourceLinesSpanLibraryFrames,
      TEST ? null : filterCallSite,
      function (_err, stacktrace, accumulatedTime) {
        // _err from gatherStackTrace is always null.
        agent._accumulateAgentTime(accumulatedTime, 'gatherStackTrace')
  ...
  agent._instrumentation.addEndedSpan(this)
  this.transaction._captureBreakdown(this)
  agent._accumulateAgentTimeFrom(s, 'Span.end')
}
...
Span.prototype._encode = function (cb) {
  ...
  function done (err, frames) {
    ...
    self._agent._accumulateAgentTimeFrom(s, 'Span._encode')
    cb(null, payload)
  }
}
