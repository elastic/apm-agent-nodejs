...
Span.prototype.end = function (endTime) {
  ...
  if (this._excludeServiceTarget || !this._exitSpan) {
    ...
    if (!this._serviceTarget.name) {
      ...
      // If this is not a DB or messaging span, and we have an `http.url`,
      // then do *not* put `.type` in the resource string. I.e. we want
      // "host:port" rather than "http/host:port".
      this._destination.service.resource = this._serviceTarget.name
    } else {
