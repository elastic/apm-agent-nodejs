...
Span.prototype.end = function (endTime) {
  ...
  if (this._excludeServiceTarget || !this._exitSpan) {
    ...
    // XXX if s3 instrumentation explicitly sets just service.target.type (no name),
    //     then I don't think this code path should go. Hrm.
    if (!('name' in this._serviceTarget)) {
      // XXX consider switching on `span.type`.
      if (this._db) {
        // XXX sylvain Q2
        ...
        //    We could enhance `_setHttpContext` to pass a separate `host` argument
        //    for possible future usage here.
        try {
          // XXX Does the spec want default port fallback (':80' for http, ':443' for https)?
    ...
    this._destination.service = {
      // These two could be skipped if the upstream APM server is known to be >=7.14.
      type: '',
      name: ''
    }
    if (!this._serviceTarget.name) {
      ...
      // have the same value as historically?) where we do NOT use the "$type/"
      // prefix. One example is HTTP spans. Another is gRPC spans and, I infer
      // from otel_bridge.feature, any OTel "rpc.system"-usage spans as well
 ...
 * instrumentation.
 *
 * @deprecated Internal code should use `_setDestinationContext()`.
 * XXX Rethink this? Also mention setServiceTarget()?
 */
Span.prototype.setDestinationContext = function (destCtx) {
  // XXX add a "next major" issue to drop this and the other internal not-`_`-prefixed context methods
  // XXX add a real "only printed once" warning for usage of this
  console.log(' XXX setDestinationContext is deprecated, deal')
...
Span.prototype._setDestinationContext = function (destCtx) {
  assert(!destCtx.service, 'XXX internal code must not set context.destination.service manually')
  this._destination = Object.assign(this._destination || {}, destCtx)
}

// XXX maybe move this to GenericSpan depending on Q to Sylvain
/**
