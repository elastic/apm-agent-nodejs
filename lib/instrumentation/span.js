...
function Span (transaction, name, ...args) {
  ...
  // console.warn('XXX new Span(name=%s, args=%s): defaultChildOf=', name, args, defaultChildOf.constructor.name, defaultChildOf.name, defaultChildOf.ended ? '.ended' : '')
...
Span.prototype.end = function (endTime) {
  ...
  // XXX yuck, when is this needed?
  //     "nested transactions" fabricated test case hits this case (mismatch of
  //     transaction); but it has no effect. I see no point in *recovering
  //     the span's transaction* at `span.end()`. Is there a real use case?
  //     See if there was when this code was added:
  //      commit f9d15b55fc469edccdf91878327f8b75a49ff3d1 from 5y ago
  //     DB connection internal pools. Hopefully we have test cases for this!
  //     This may be bunk, at least for pg.test.js the _pulse* wrapper was placebo.
  //
  //     And ugh, I hope we don't need to have this hack to cope with those.
  //     The test cases updated there are the "recoverable*" ones in integration/index.test.js.
  //     No *new* test cases were added.
  //     - Another case that hits this is "with app sub-span extending beyond end"
  //       in breakdown.test.js where span.end() happens after tx.end(). At
  //       least with the new ctxmgr work, the span ends and is serialized
  //       fine without "currentTransaction", so is all fine.
...
// XXX What happens if we remove this for all tests using (the uncommented!!) ELASTIC_APM_TEST?
