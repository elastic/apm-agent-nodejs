...
Transaction.prototype.end = function () {
  ...
  this._client.logger.trace('ended transaction (name: %s, type: %s, result: %s)', this.name, this.type, this.result)
}

Transaction.prototype.startTrace = function (signature, type) {
  var trace = new Trace(this, signature, type)
  this._client.logger.trace('start trace (transaction: %s, signature: %s, type: %s)', this.name, signature, type)
...
Transaction.prototype._endTrace = function (trace) {
  if (this.ended) {
    this._client.logger.error('Can\'t end trace after parent transaction have ended - ignoring!')
    return
  }
  ...
  this._client.logger.trace('ended trace (transaction: %s, signature: %s, type: %s)', trace.transaction.name, trace.signature, trace.type)
}
