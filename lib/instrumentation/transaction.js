...
// getPathFromRequest relies on express instrumentation being loaded
// for the case where instrumentation is not loaded and not path
// is returned, cautiously return the first URL segment as the path
// name in order to provide a reasonable default.
// See: https://github.com/elastic/apm-agent-nodejs/issues/2288
function getSafePathFromReq (req) {
  if (!req.url) {
    return
  }
...
Transaction.prototype.setDefaultNameFromRequest = function () {
  ...
  if (!path && !this._agent._conf.instrument) {
    path = getSafePathFromReq(req)
  }
