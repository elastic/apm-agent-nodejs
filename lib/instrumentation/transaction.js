...
Transaction.prototype.end = function () {
  ...
  this._client.logger.trace('[%s] ended transaction (name: %s, type: %s, result: %s)', this._uuid, this.name, this.type, this.result)
}

Transaction.prototype.startTrace = function (signature, type) {
  var trace = new Trace(this, signature, type)
  this._client.logger.trace('[%s] start trace (transaction: %s, signature: %s, type: %s)', this._uuid, this.name, signature, type)
...
Transaction.prototype._endTrace = function (trace) {
  if (this.ended) {
    this._client.logger.error('[%s] Can\'t end trace after parent transaction have ended - ignoring!', this._uuid)
    return
  }
  ...
  this._client.logger.trace('[%s] ended trace (transaction: %s, signature: %s, type: %s)', this._uuid, trace.transaction.name, trace.signature, trace.type)
}
