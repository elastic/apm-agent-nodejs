...
function Transaction (agent, name, ...args) {
  ...
  agent.logger.debug({ span: this }, '%s trace', verb)
...
Object.defineProperty(Transaction.prototype, 'name', {
  ...
  set (name) {
    if (this.ended) {
      this._agent.logger.debug({ span: this }, 'tried to set transaction.name on already ended transaction')
      return
    }
    this._agent.logger.debug({ span: this }, 'setting transaction name: %s', name)
...
Object.defineProperty(Transaction.prototype, 'result', {
  ...
  set (result) {
    if (this.ended) {
      this._agent.logger.debug({ span: this }, 'tried to set transaction.result on already ended transaction')
      return
    }
    this._agent.logger.debug({ span: this, result }, 'setting transaction result')
    this._result = result
  }
})
...
Transaction.prototype.startSpan = function (name, ...args) {
  ...
  if (this.ended) {
    this._agent.logger.debug({ span: this }, 'transaction already ended - cannot build new span') // TODO: Should this be supported in the new API?
    return null
  }
...
Transaction.prototype._encode = function () {
  if (!this.ended) {
    this._agent.logger.error({ span: this }, 'cannot encode un-ended transaction')
    return null
  }
...
Transaction.prototype.setDefaultName = function (name) {
  this._agent.logger.debug({ span: this }, 'setting default transaction name: %s', name)
  this._defaultName = name
}
...
Transaction.prototype.setDefaultNameFromRequest = function () {
  ...
  if (!path) {
    this._agent.logger.debug({
      span: this,
      url: req.url,
      pathIsNull: path === null, // because typeof null === 'object'
      typeOfPath: typeof path,
      hasRoute: !!req.route,
      isRegex: req.route ? !!req.route.regexp : false,
...
Transaction.prototype.end = function (result, endTime) {
  if (this.ended) {
    this._agent.logger.debug({ span: this }, 'tried to call transaction.end() on already ended transaction')
    return
  }
  ...
  if (!trans) {
    this._agent.logger.debug({ current: trans, spans: this._builtSpans, span: this }, 'WARNING: no currentTransaction found')
    this._contextLost = true
  } else if (trans !== this) {
    this._agent.logger.debug({ other: trans.id, spans: this._builtSpans, span: this }, 'WARNING: transaction is out of sync')
    this._contextLost = true
  }

  this._agent._instrumentation.addEndedTransaction(this)
  this._agent.logger.debug({ span: this, result: this.result }, 'ended transaction')
}
