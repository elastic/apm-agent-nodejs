'use strict'

var uuid = require('node-uuid')
var Trace = require('./trace')

var Transaction = module.exports = function (agent, name, type, result) {
  Object.defineProperty(this, 'name', {
    configurable: true,
    enumerable: true,
    get: function () {
      return this._customName || this._defaultName
    },
    set: function (name) {
      this._agent.logger.trace('[%s] setting transaction name: %s', this._uuid, name)
      this._customName = name
    }
  })

  this._defaultName = name
  this.type = type
  this.result = result
  this.traces = []
  this.ended = false
  this._uuid = uuid.v4()
  this._agent = agent
  this._agent._instrumentation.currentTransaction = this

  this._agent.logger.trace('[%s] start transaction (name: %s, type: %s, result: %s)', this._uuid, name, type, result)

  // Keep track of which traces overlap other traces. Contains start/stop
  // events in form of arrays: `[Boolean, Trace]`. The `Boolean` is false if
  // the Trace just started and true if it just ended.
  this._tracesStartStopOrder = []

  // A transaction should always have a root trace spanning the entire
  // transaction.
  this._rootTrace = this.startTrace('transaction', 'transaction')
  this._start = this._rootTrace._start
  this.duration = this._rootTrace.duration.bind(this._rootTrace)
}

Transaction.prototype.end = function () {
  if (this.ended) return this._agent.logger.warn('[%s] cannot end already ended transaction', this._uuid)

  this._rootTrace.end()
  this.ended = true

  if (this !== this._agent._instrumentation.currentTransaction) {
    // This should normally not happen, but if the hooks into Node.js doesn't
    // work as intended it might. In that case we want to gracefully handle it.
    // That involves ignoring all traces under the given transaction as they
    // will most likely be incomplete. We still want to send the transaction
    // without any traces to Opbeat as it's still valuable data.
    this._agent.logger.warn('[%s] transaction is out of sync', this._uuid)
    this.traces = []
  }

  this._agent._instrumentation.addEndedTransaction(this)
  this._agent.logger.trace('[%s] ended transaction (type: %s, result: %s)', this._uuid, this.type, this.result)
}

Transaction.prototype.startTrace = function (signature, type) {
  var trace = new Trace(this, signature, type)
  this._latestTrace = trace
  this._tracesStartStopOrder.push([false, trace])
  return trace
}

Transaction.prototype._recordEndedTrace = function (trace) {
  if (this.ended) {
    this._agent.logger.error('[%s] Can\'t record ended trace after parent transaction have ended - ignoring!', this._uuid)
    return
  }

  this._tracesStartStopOrder.push([true, trace])
  this.traces.push(trace)

  // a simple way to compare two traces within a Transaction and see if one
  // ended before or after another
  trace._endOrder = this._tracesStartStopOrder.length
}
