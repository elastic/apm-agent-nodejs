...
function Transaction (agent, name, ...args) {
  ...
  this.outcome = constants.TRANSACTION.OUTCOME.UNKNOWN
}
...
Transaction.prototype.setOutcome = function (outcome) {
  if (outcome !== constants.TRANSACTION.OUTCOME.FAILURE &&
    outcome !== constants.TRANSACTION.OUTCOME.SUCCESS &&
    outcome !== constants.TRANSACTION.OUTCOME.UNKNOWN) {
...
Transaction.prototype._setOutcomeFromStatusCode = function (statusCode) {
  ...
  if (statusCode >= 500) {
    this.outcome = constants.TRANSACTION.OUTCOME.FAILURE
  } else {
    this.outcome = constants.TRANSACTION.OUTCOME.SUCCESS
  }
}
