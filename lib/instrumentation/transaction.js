...
function Transaction (agent, name, type, result) {
  Object.defineProperty(this, 'name', {
    ...
    set: function (name) {
      debug('setting transaction name', { uuid: this._uuid, name: name })
  ...
  debug('start transaction', { uuid: this._uuid, name: name, type: type, result: result })
...
Transaction.prototype.end = function () {
  if (this.ended) return debug('cannot end already ended transaction', { uuid: this._uuid })
  ...
  if (!trace || trace.transaction !== this) {
    ...
    debug('transaction is out of sync', { uuid: this._uuid })
    this.traces = []
  }

  this._agent._instrumentation.addEndedTransaction(this)
  debug('ended transaction', { uuid: this._uuid, type: this.type, result: this.result })
}

Transaction.prototype._recordEndedTrace = function (trace) {
  if (this.ended) {
    debug('Can\'t record ended trace after parent transaction have ended - ignoring', { uuid: this._uuid })
    return
  }
