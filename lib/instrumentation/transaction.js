...
function Transaction (agent, name, ...args) {
  ...
  agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId, name: this.name, type: this.type, subtype: this.subtype, action: this.action }, '%s trace', verb)
...
Object.defineProperty(Transaction.prototype, 'name', {
  ...
  set (name) {
    if (this.ended) {
      this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'tried to set transaction.name on already ended transaction')
      return
    }
    this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId, name }, 'setting transaction name')
...
Object.defineProperty(Transaction.prototype, 'result', {
  ...
  set (result) {
    if (this.ended) {
      this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'tried to set transaction.result on already ended transaction')
      return
    }
    this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId, result }, 'setting transaction result')
    this._result = result
  }
})
...
Transaction.prototype.startSpan = function (name, ...args) {
  ...
  if (this.ended) {
    this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'transaction already ended - cannot build new span') // TODO: Should this be supported in the new API?
    return null
  }
...
Transaction.prototype._encode = function () {
  if (!this.ended) {
    this._agent.logger.error({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'cannot encode un-ended transaction')
    return null
  }
...
Transaction.prototype.setDefaultName = function (name) {
  this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'setting default transaction name: %s', name)
  this._defaultName = name
}
...
Transaction.prototype.end = function (result, endTime) {
  if (this.ended) {
    this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId }, 'tried to call transaction.end() on already ended transaction')
    return
  }
  ...
  if (!trans) {
    this._agent.logger.debug({ current: trans, spans: this._builtSpans, trans: this.id, parent: this.parentId, trace: this.traceId }, 'WARNING: no currentTransaction found')
    this._contextLost = true
  } else if (trans !== this) {
    this._agent.logger.debug({ other: trans.id, spans: this._builtSpans, trans: this.id, parent: this.parentId, trace: this.traceId }, 'WARNING: transaction is out of sync')
    this._contextLost = true
  }

  this._agent._instrumentation.addEndedTransaction(this)
  this._agent.logger.debug({ trans: this.id, parent: this.parentId, trace: this.traceId, type: this.type, result: this.result, name: this.name }, 'ended transaction')
}
