...
function Transaction (agent, name, ...args) {
  ...
  if (opts.childOf) {
    ...
    if (traceContinuationStrategy === TRACE_CONTINUATION_STRATEGY_RESTART) {
      const link = spanLinkFromChildOf(opts.childOf)
      if (link) {
...
// XXX might want this somewhere else to share
// Return a span link object for the given `childOf` argument to Transaction
// or Span creation. This returns null if `childOf` is not given or is
// invalid.
//
// @param {Transaction | Span | TraceParent | string} [childOf] - Span link
//    data is extracted from this argument. If this is a string, it is
//    expected to be a W3C trace-context 'traceparent' string.
function spanLinkFromChildOf (childOf) {
  let traceId
  let spanId

  if (!childOf) {
    return null
  } else if (childOf._context instanceof TraceContext) {
    traceId = childOf._context.traceparent.traceId
    spanId = childOf._context.traceparent.id
  } else if (childOf instanceof TraceParent) {
    traceId = childOf.traceId
    spanId = childOf.id
  } else if (typeof (childOf) === 'string') {
    // Note: Unfortunately this doesn't validate the string.
    const traceparent = TraceParent.fromString(childOf)
