...
Transaction.prototype.startSpan = function (name, ...args) {
  ...
  const span = new Span(this, name, ...args)
  this._agent._instrumentation.enterSpanRunContext(span)
  return span
}
...
Transaction.prototype.end = function (result, endTime) {
  ...
  const currTrans = this._agent._instrumentation.currTransaction()
  if (!currTrans) {
    this._agent.logger.debug('WARNING: no currentTransaction found %o', { current: currTrans, spans: this._builtSpans, trans: this.id, parent: this.parentId, trace: this.traceId })
  } else if (currTrans !== this) {
    this._agent.logger.debug('WARNING: transaction is out of sync %o', { other: currTrans.id, spans: this._builtSpans, trans: this.id, parent: this.parentId, trace: this.traceId })
  }
