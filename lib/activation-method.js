...
/**
 * Determine the 'system.agent.activation_method' metadata value from an Error
...
function agentActivationMethodFromStartStack (startStack, log) {
  ...
  try {
    ...
    log.trace(parseErr, 'could not determine metadata.system.agent.activation_method')
    return 'unknown'
  }
  if (frames.length < 2) {
    return 'unknown'
  ...
  const functionName = (semver.gte(process.version, '12.0.0', { includePrerelease: true })
    ? 'loadPreloadModules'
    : 'preloadModules')
  let isPreload = false
