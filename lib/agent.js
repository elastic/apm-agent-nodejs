...
Agent.prototype.start = function (opts) {
  ...
  if (!this.active) {
    logger.info('Opbeat logging is disabled for now')
    return this
  } else if (!this.appId || !this.organizationId || !this.secretToken) {
    logger.info('[WARNING] Opbeat logging is disabled. To enable, specify organization id, app id and secret token')
    this.active = false
    return this
  }
  ...
  this.on('logged', function (url, uuid) {
    logger.info('[%s] Opbeat logged error successfully at %s', uuid, url)
  })
...
Agent.prototype.captureError = function (err, data, cb) {
  ...
  if (!util.isError(err)) {
    ...
    logger[level]('[%s]', errUUID, data.message)
  ...
  if (!isMessage) {
    logger.info('[%s] logging error with Opbeat:', errUUID)
...
Agent.prototype.handleUncaughtExceptions = function (cb) {
  ...
  this._uncaughtExceptionListener = function (err) {
    ...
    logger.debug('[%s] Opbeat caught unhandled exception', data.extra.uuid)
    ...
    agent.captureError(err, data, function (opbeatErr, url) {
      if (opbeatErr) {
        logger.info('[%s] Could not notify Opbeat!', data.extra.uuid)
        logger.error(opbeatErr.stack)
      } else {
        logger.info('[%s] Opbeat logged error successfully at %s', data.extra.uuid, url)
      }
...
Agent.prototype.trackRelease = function (data, cb) {
  var agent = this
  if (data.path) {
    logger.warn('Detected use of deprecated path option to trackRelease function!')
...
function errorLogger (err, uuid) {
  if (uuid) logger.info('[%s] Could not notify Opbeat!', uuid)
  else logger.info('Could not notify Opbeat!')
