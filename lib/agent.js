...
Agent.prototype.start = function (opts) {
  ...
  if (!this._conf.serverUrl) {
    this.logger.error('Elastic APM is incorrectly configured: Invalid serverUrl (APM will be disabled)')
...
Agent.prototype.setGlobalLabel = function (key, value) {
  ...
  if (!this._apmClient) {
    this.logger.warn('cannot add global label to inactive or unconfigured agent')
    return
  }
