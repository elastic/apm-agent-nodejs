...
Agent.prototype.start = function (opts) {
  ...
  if (this._conf.opentelemetrySdk) {
    const { OTelBridgeRunContext } = require('./opentelemetry-sdk/OTelBridgeRunContext')
...
Agent.prototype._startOTelBridge = function () {
  ...
  // XXX s/Elastic/OTel
  const { OTelContextManager } = require('./opentelemetry-sdk/OTelContextManager')
  const { OTelPropagator } = require('./opentelemetry-sdk/OTelPropagator')
  const { OTelTracerProvider } = require('./opentelemetry-sdk/OTelTracerProvider')
  const { OTelTracer } = require('./opentelemetry-sdk/OTelTracer')
  ...
  success = api.trace.setGlobalTracerProvider(new OTelTracerProvider(new OTelTracer(this)))
  if (!success) {
  ...
  // be used.
  const { fetchSpanKey } = require('./opentelemetry-sdk/OTelBridgeRunContext')
  fetchSpanKey()
  ...
  // XXX Is this needed?
  success = api.propagation.setGlobalPropagator(new OTelPropagator(this))
  if (!success) {
