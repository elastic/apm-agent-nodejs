...
const { isOTelMeterProviderSupported, createOTelMeterProvider } = require('./opentelemetry-metrics')
...
Agent.prototype.destroy = function () {
  ...
  // XXX Does this break because the transport was destroyed above already!?
  // Dev Note: I don't like that this can be doing work *after* `.destroy()`
  // returns, but there isn't a way around it.
  // XXX Perhaps open a 'breaking change' issue to make Agent.destroy() async.
  if (this._otelMeterProvider) {
    this._otelMeterProvider.shutdown({ timeoutMillis: 1000 })
      .catch(reason => {
...
Agent.prototype.start = function (opts) {
  ...
  this._agentActivationMethod = agentActivationMethodFromStartStack(startStack, this.logger)
  ...
  if (this._conf.logLevel === 'trace') {
    ...
    this.logger.trace({
      ...
      agent: version,
      agentActivationMethod: this._agentActivationMethod,
  ...
  const isMetricsEnabled = this._conf.metricsInterval !== 0 && !this._conf.contextPropagationOnly
  if (isMetricsEnabled) {
    this._metrics.start()
  }
