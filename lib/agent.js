...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  // XXX REVIEW-NOTE: This changes handled to be set `true` in cases where it
  //    was sometimes left `undefined` by (I think) accident/bug of setting it
  //    `handled = !opts || opts.handled`. The result of the latter is that
  //    adding unrelated opts to have `captureError(err, { request: res}, cb)`
  //    changes `handled` from true to undefined.
  ...
  setImmediate(() => {
    ...
    if (res) {
      // XXX I'm curious if current tests would have caught this sync/async buglet in incomplete res.
