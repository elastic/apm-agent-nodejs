...
Agent.prototype.start = function (opts) {
  ...
  if (isMetricsEnabled) {
    const isOTelMetricsSdkSupported = semver.satisfies(process.versions.node, '>=14')
    if (isOTelMetricsSdkSupported) {
      const { setupOTelMeterProvider } = require('./opentelemetry-metrics')
      setupOTelMeterProvider(this)
    }
    this._metrics.start()
  }
