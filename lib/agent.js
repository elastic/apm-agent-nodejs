...
Agent.prototype.setBaggageEntry = function (key, entry) {
  // TODO: What to do if there is no current transaction?
  return this._instrumentation.setBaggageEntry(key, entry)
}
Agent.prototype.getBaggageEntry = function (key) {
  return this._instrumentation.getBaggageEntry(key)
}
...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  var context = (span || trans || {})._context || { traceparent: {} }
  ...
  function prepareError (error) {
    ...
    error.parent_id = context.traceparent.id
    error.trace_id = context.traceparent.traceId
