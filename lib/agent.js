...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  function send (error) {
    ...
    if (!error) {
      agent.logger.debug({ id }, 'error ignored by filter')
      if (cb) cb(null, id)
      return
    }

    if (agent._transport) {
      agent.logger.info({ id }, 'Sending error to Elastic APM')
...
Agent.prototype.flush = function (cb) {
  if (this._transport) {
    ...
    this.logger.warn({ err: new Error('cannot flush agent before it is started') })
