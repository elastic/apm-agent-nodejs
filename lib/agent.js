...
const { agentInstallationMethodFromStartStack } = require('./agent-installation-method')
...
Agent.prototype.start = function (opts) {
  ...
  Error.stackTraceLimit = 20 // require more than default 10 for `agentInstallationMethodFromStartStack()`
  const startStack = {}
  Error.captureStackTrace(startStack)
  this._agentInstallationMethod = agentInstallationMethodFromStartStack(startStack, this.logger)
  ...
  if (this._conf.logLevel === 'trace') {
    ...
    this.logger.trace({
      ...
      agent: version,
      agentInstallationMethod: this._agentInstallationMethod,
