...
Object.defineProperty(Agent.prototype, 'currentTransaction', {
  get () {
    return this._instrumentation.currTx()
  }
})
...
Agent.prototype.setSpanOutcome = function (outcome) {
  ...
  if (!span) {
    this.logger.debug('no active span found - cannot set span outcome')
    return null
  }
...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  if (!this._transport) {
    if (cb) {
      process.nextTick(cb,
        new Error('cannot capture error before agent is started'),
