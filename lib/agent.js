...
var { elasticApmAwsLambda, isLambdaExecutionEnvironment } = require('./lambda')
 ...
 *
 * | Install/start method                                | `agent.installation.method`          |
 * | --------------------------------------------------- | ------------------------------------ |
 * | `require('elastic-apm-node').start(...)`            | "require-and-start"                  |
 * | `require('elastic-apm-node/start')`                 | "require-start"                      |
 * | `import 'elastic-apm-node/start.js'`                | "import-start"                       |
 * | `import apm from 'elastic-apm-node'; apm.start()`   | "import-and-start"                   |
 * | Using [AWS Lambda Layer][]                          | "preload-require-start-lambda-layer" |
 * | Via [APM Attacher][]                                | "preload-require-start-apm-attacher" |
 * | `node -r elastic-apm-node/start ...`                | "preload-require-start"              |
 * | `NODE_OPTIONS='-r elastic-apm-node/start' node ...` | "preload-require-start-envvar"       |
 *
 * Any of the `*-{require,import}-start-*` names can have a "start-next" variant if
 * the `elastic-apm-node/start-next.js` start module is used during the Next.js
 * instrumentation tech preview. This "start-next.js" module is expected to be
 * removed when Next.js instr goes GA.
 */
function startMethodFromStartStack (startStack, log) {
  /* @param {require('stackframe').StackFrame[]} frames */
  let frames
  try {
    frames = errorStackParser.parse(startStack)
  } catch (parseErr) {
    log.trace('could not determine install/start method: %s', parseErr)
    return 'unknown'
  }
  if (frames.length < 2) {
    return 'unknown'
  }
  console.log('XXX frames: ', frames)
  ...
  console.log('XXX topDir: ', topDir)

  // frames[1] tells us if "start[-next].js" module was used, e.g.:
  //    at Object.<anonymous> (/Users/trentm/tmp/asdf/node_modules/elastic-apm-node/start.js:9:32)
  let usedStartModule = null
  const p = path.parse(frames[1].fileName)
  if (p.dir === topDir) {
    if (p.base === 'start.js') {
      usedStartModule = 'start'
    } else if (p.base === 'start-next.js') {
      usedStartModule = 'start-next'
    }
  }
  console.log('XXX usedStartModule: ', usedStartModule)

  // To tell if elastic-apm-node was `import`d or `require`d we look for a
  // fram with `functionName: 'async ESMLoader.import'`.
  ...
  console.log('XXX isImport: ', isImport)
  if (isImport) {
    if (usedStartModule) {
      return `import-${usedStartModule}`
    } else {
      return 'import-and-start'
    }
  }

  if (!usedStartModule) {
    return 'require-and-start'
  }
  ...
  if (frames[frames.length - 2].functionName === 'loadPreloadModules') {
    if (isLambdaExecutionEnvironment && topDir === '/opt/nodejs/node_modules/elastic-apm-node') {
      return `preload-require-${usedStartModule}-lambda-layer`
      ...
      return `preload-require-${usedStartModule}-apm-attacher`
    } else if (process.env.NODE_OPTIONS &&
        CONTAINS_R_ELASTIC_APM_NODE_START.test(process.env.NODE_OPTIONS)) {
      return `preload-require-${usedStartModule}-envvar`
    } else {
      return `preload-require-${usedStartModule}`
    }
  }

  if (usedStartModule) {
    return `require-${usedStartModule}`
  }
...
Agent.prototype.start = function (opts) {
  ...
  this._startMethod = startMethodFromStartStack(startStack, this.logger)
  console.log('XXX startMethod: ', this._startMethod)
  ...
  if (this._conf.logLevel === 'trace') {
    ...
    this.logger.trace({
      ...
      agent: version,
      startMethod: this._startMethod,
      startTrace: startStack.stack.split(/\n */).slice(1),
