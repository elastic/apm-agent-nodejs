...
function Agent () {
  ...
  this._accumulatedAgentTime = 0
  this._agentTimeFromWhat = {}
  this._lastCpuUsageMs = 0
...
Agent.prototype._getStats = function () {
  const cpuUsage = this._cpuUsageSinceLast()

  const stats = {
    frameCache: frameCacheStats,
    agentOverhead: this._accumulatedAgentTime / cpuUsage,
    accumulatedAgentTime: this._accumulatedAgentTime,
    cpuUsage,
    agentTimeFromWhat: this._agentTimeFromWhat
  }
  if (this._transport && typeof this._transport._getStats === 'function') {
    stats.apmclient = this._transport._getStats()
  }

  // XXX reset agent-time accumulation
  this._accumulatedAgentTime = 0
  this._agentTimeFromWhat = {}
...
// `s` is a performance.now() start time. The time from that start to now
// is added to the agent-time accumulator. This accumulator is used to
// approximately measure agent CPU overhead. `_what` is a short string to
// document what Agent work is being accumulated -- currently only used for
// self-documentation.
Agent.prototype._accumulateAgentTimeFrom = function (s, what) {
  const d = performance.now() - s
  this._accumulatedAgentTime += d
  if (!(what in this._agentTimeFromWhat)) { // XXX could avoid this one with registry
    this._agentTimeFromWhat[what] = d
  } else {
    this._agentTimeFromWhat[what] += d
  }
}
Agent.prototype._accumulateAgentTime = function (d, what) {
  this._accumulatedAgentTime += d
  if (!(what in this._agentTimeFromWhat)) { // XXX could avoid this one with registry
    this._agentTimeFromWhat[what] = d
  } else {
...
// Return the CPU usage in milliseconds (adding 'system' and 'user') since the
// last call to this function.
Agent.prototype._cpuUsageSinceLast = function () {
  const u = process.cpuUsage()
  const usageMs = (u.system + u.user) / 1000
  const usageSinceLast = usageMs - this._lastCpuUsageMs
  this._lastCpuUsageMs = usageMs
