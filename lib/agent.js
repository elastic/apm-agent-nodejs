...
Agent.prototype.start = function (opts) {
  ...
  let runContextClass
  if (this._conf.opentelemetryBridge) {
    const { OTelBridgeRunContext } = require('./otelbridge/OTelBridgeRunContext')
    runContextClass = OTelBridgeRunContext
    this._startOTelBridge()
  }
...
// XXX move to lib/otel/setup.js
Agent.prototype._startOTelBridge = function () {
  ...
  // XXX s/Elastic/OTel
  const { OTelContextManager } = require('./otelbridge/ElasticContextManager')
  const { ElasticPropagator } = require('./otelbridge/ElasticPropagator')
  const { ElasticTracerProvider } = require('./otelbridge/ElasticTracerProvider')
  const { ElasticTracer } = require('./otelbridge/ElasticTracer')

  // XXX hook into this.logger, if can.
  ...
  if (!success) {
    this.logger.error('could not register OpenTelemetry bridge diag logger')
    return
  }

  success = api.trace.setGlobalTracerProvider(new ElasticTracerProvider(new ElasticTracer(this)))
  if (!success) {
    this.logger.error('could not register OpenTelemetry bridge TracerProvider')
    return
  }

  // The OTelBridgeRunContext class needs to get the SPAN_KEY before it can
  // be used.
  const { fetchSpanKey } = require('./otelbridge/OTelBridgeRunContext')
  fetchSpanKey()

  success = api.context.setGlobalContextManager(new OTelContextManager(this))
  if (!success) {
    this.logger.error('could not register OpenTelemetry bridge ContextManager')
    return
  }

  // XXX Is this needed?
  success = api.propagation.setGlobalPropagator(new ElasticPropagator(this))
  if (!success) {
    this.logger.error('could not register OpenTelemetry bridge Propagator')
  }
}
