...
const { cloudMetadataFetcher, isValidCloudProviderOption, VALID_CLOUD_PROVIDERS_OPTIONS } = require('./cloud')
...
var VALIDATORS = {
  ...
  cloudProvider: isValidCloudProviderOption
}
...
var ENUM_OPTS = [
  ['cloudProvider', VALID_CLOUD_PROVIDERS_OPTIONS]
]
    ...
    if (typeof this.transport !== 'function') {
      this.transport = function httpTransport (conf, agent) {
        var transport = new ElasticAPMHttpClient({
          ...
          cloudMetadataFetcher: conf.cloudProvider && cloudMetadataFetcher(agent, conf.cloudProvider),
...
function normalizeEnums (opts) {
  for (const [key, validEnumValues] of ENUM_OPTS) {
    if (key in opts) opts[key] = strictEnumValue(opts.logger, key, validEnumValues, opts[key])
  }
}
...
function strictEnumValue (logger, key, validEnumValues, value) {
  const valueFound = validEnumValues.find(validValue => {
    return validValue === value || (typeof validValue === 'boolean' && validValue.toString() === value)
  })

  if (!value) {
    logger.warn('unrecognized value "%s" for "%s", valid values are: %s', value, key, validEnumValues.join(', '))
    return
  }
