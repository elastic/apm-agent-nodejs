...
const { getCloudMetadata } = require('./instrumentation/cloud-metadata')
...
class Config {
  constructor (opts) {
    ...
    // Create a logger if a custom `logger` was not provided.
    ...
    if (typeof this.transport !== 'function') {
      this.transport = function httpTransport (conf, agent) {
        // const cloudMetadataFetcher = false
        // uncomment to pass the cloudMetadataFetcher to the clinet
        const cloudMetadataFetcher = getCloudMetadata
        var transport = new ElasticAPMHttpClient({
          ...
          cloudMetadataFetcher: cloudMetadataFetcher
        })
