...
function normalizeBools (opts, logger) {
  for (const key of BOOL_OPTS) {
    const isKeyPresent = key in opts

    if (isKeyPresent) opts[key] = strictBool(logger, key, opts[key])
    // TODO: remove this log in the scope of
    // https://github.com/elastic/apm-agent-nodejs/issues/3332
    if (isKeyPresent && key === 'filterHttpHeaders') {
