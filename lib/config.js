...
var DEFAULTS = {
  ...
  sanitizeFieldNames: ['password', 'passwd', 'pwd', 'secret', '*key', '*token*',
    '*session*', '*credit*', '*card*', '*auth*', 'set-cookie',
    'pw', 'pass', 'connect.sid'
  ],
...
var ENV_TABLE = {
  ...
  ignoreMessageQueues: ['ELASTIC_APM_IGNORE_MESSAGE_QUEUES','ELASTIC_IGNORE_MESSAGE_QUEUES'],
  ...
  sanitizeFieldNames: ['ELASTIC_APM_SANITIZE_FIELD_NAMES', 'ELASTIC_SANITIZE_FIELD_NAMES'],
...
function readEnv () {
  ...
  // // legacy keys for backward compatability
  // const legacyKeys = {
...
// getPkg() attempts to find a "package.json" file (looking up from the current
// working directory), read it, and return an object of the form:
//    {
//      path: "/the/full/path/to/package.json",
//      packageJson: { /* the loaded package.json object */ }
//    }
let pkgCache
function getPkg () {
  if (pkgCache === undefined) {
    pkgCache = readPkgUp.sync() || null
  }
  return pkgCache
}
...
function serviceNameFromPackageJson () {
  const pkg = getPkg()
  if (!pkg) {
    throw new Error(`could not infer serviceName: could not find package.json up from ${process.cwd()}`)
  }
  if (!pkg.packageJson.name) {
    throw new Error(`could not infer serviceName: "${pkg.path}" does not contain a "name"`)
  }
  if (typeof pkg.packageJson.name !== 'string') {
    throw new Error(`could not infer serviceName: "name" in "${pkg.path}" is not a string`)
  }
  let serviceName = pkg.packageJson.name
  ...
  if (!serviceName) {
    throw new Error(`could not infer serviceName from name="${pkg.packageJson.name}" in "${pkg.path}"`)
  }
...
function serviceVersionFromPackageJson () {
  const pkg = getPkg()
  if (!pkg) {
    throw new Error(`could not infer serviceVersion: could not find package.json up from ${process.cwd()}`)
  }
  if (!pkg.packageJson.version) {
    throw new Error(`could not infer serviceName: "${pkg.path}" does not contain a "version"`)
  }
  if (typeof pkg.packageJson.version !== 'string') {
    throw new Error(`could not infer serviceName: "version" in "${pkg.path}" is not a string`)
  }
