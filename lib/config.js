...
function normalizeIgnoreOptions (opts) {
  if (opts.transactionIgnoreUrls) {
    // if users accidently configure a string instead of an
    // array with a single string, handle it for them
    if (!Array.isArray(opts.transactionIgnoreUrls)) {
      opts.transactionIgnoreUrls = [opts.transactionIgnoreUrls]
    }

    // We can't guarentee that opts will be a Config so set a
    ...
    for (let ptn of opts.transactionIgnoreUrls) {
      // if configured without a leading '/', be nice
      // and add this to pattern for the user
      if (ptn[0] !== '/') ptn = '/' + ptn
