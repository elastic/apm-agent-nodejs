...
class Config {
  constructor (opts, logger) {
    ...
    // Food 4 thought: We may expand this to warn about others if we add more info to the options itself
    [envOptions, opts, confFile].forEach(function (options) {
      if (options && 'filterHttpHeaders' in options) {
        logger.warn('the `filterHttpHeaders` config option is deprecated')
      }
    })
...
/**
 * Normalize and validate the given values for boolean configuration properties
 *
 * - the `filterHttpHeaders` config option is now deprecated. The default values of
 *   `sanitizeFieldNames` already provide the same filtering of senstive information
 *   within the `cookie` info
 */
