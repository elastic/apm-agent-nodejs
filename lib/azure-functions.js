...
function getAzureAccountId () {
  return process.env.WEBSITE_OWNER_NAME && process.env.WEBSITE_OWNER_NAME.split('+')[0]
}
...
function getAzureFunctionsExtraMetadata () {
  const metadata = {
    ...
    // XXX discuss on spec
    // cloud: {
    //   provider: 'azure', // XXX discuss on spec
    //   region: process.env.REGION_NAME, // XXX discuss on spec
    //   service: {
    //     name: 'azure functions' // XXX or 'functions'? Discuss on spec
    //   }
    // }
  }
  // XXX discuss on spec
