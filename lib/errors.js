...
function culpritFromStacktrace (frames) {
  ...
  let filename = frames[0].filename
...
function _moduleNameFromFrames (frames) {
  ...
  const idx = frame.filename.lastIndexOf('node_modules' + path.sep)
  if (idx === -1) {
    return null
  }
  const parts = frame.filename.slice(idx).split(path.sep)
  if (!parts[1]) {
    return null
...
function createAPMError (args, cb) {
  ...
  if (args.exception) {
    ...
    if ('code' in err) {
      ...
      const match = errMsg.match(MYSQL_ERROR_MSG_RE)
      if (match) {
