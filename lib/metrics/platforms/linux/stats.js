...
class Stats {
  ...
  readStats ([processFile, memoryFile, cpuFile]) {
    ...
    const cpuLine = cpuFile.toString().split('\n')[0]
    ...
    const processLine = processFile.toString().split('\n')[0]
  ...
  update (files) {
    ...
    const cpuProcessPercent = (utime + stime) / cpuTotal || 0
    const cpuProcessSystemPercent = stime / cpuTotal || 0
    const cpuProcessUserPercent = utime / cpuTotal || 0
