...
class Stats {
  ...
  readStats ([processFile, memoryFile, cpuFile]) {
    ...
    for (let line of cpuFile.toString().split('\n')) {
      if (/^cpu /.test(line)) {
        cpuLine = line
        break
      }
    ...
    if (cpuLine) {
      const columns = cpuLine.split(whitespace).slice(1).map(Number)
    ...
    let processLine = processFile.toString().split('\n')[0]
    processLine = processLine.slice(processLine.indexOf(')'))
    const processData = processLine.split(whitespace)
  ...
  update (files) {
    ...
    const cpuProcessPercent = (delta.utime + delta.stime) / delta.cpuTotal || 0
