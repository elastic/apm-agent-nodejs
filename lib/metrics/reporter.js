  ...
  _reportMetrics (metrics) {
    ...
    const next = afterAll(() => {
      ...
      for (const metric of metrics) {
        ...
        let value = metric.metricImpl.toJSON()

        const isHistogram = value && typeof value === 'object' && 'histogram' in value
        // histograms should be sent as { values, counts }
        if (!isHistogram) {
          value = { value }
        } else {
          if (!value.histogram.count) {
            continue
          }

          let histogram = new Histogram(1, Math.max(2, Math.ceil(value.histogram.max)))
          let histoValues = metric.metricImpl._histogram._sample.toArray()

          histoValues.forEach(val => {
            histogram.record(Math.ceil(val))
          })

          let counts = histogram.recordedcounts()
            .sort(function ( a, b ) {
              return a.value === b.value ? 0 : a.value - b.value
            })

          value = {
            values: counts.map(function ( count ) {
              return count.value
            }),
        ...
        data.samples[metric.name] = value

        if (metric.metricImpl.constructor.name === 'Counter'
          || metric.metricImpl.constructor.name === 'Timer'
        ) {
