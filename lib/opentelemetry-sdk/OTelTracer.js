...
class OTelTracer {
  ...
  startSpan (name, options = {}, otelContext = otel.context.active()) {
    osdklog.apicall('OTelTracer.startSpan(name=%s, options=%j, context=%s)', name, options, otelContext)
    ...
    if (otelContext instanceof OTelBridgeRunContext) {
      ...
      // XXX Drop this else-if if lifting the Transaction#startSpan() restriction on trans.ended works and passes tests.
      //    XXX also try start-span-with-context.js sent to a real APM Server to see if the span_count.started being low is a problem
      // } else if (parentGenericSpan instanceof Transaction && parentGenericSpan.ended) {
      //   // XXX Discussing this. This would be a tweak to the OTel Bridge spec.
      //   //    https://github.com/elastic/apm/blob/main/specs/agents/tracing-api-otel.md#spans-and-transactions
      //   // The parent is an *ended Transaction*. The agent doesn't allow
      //   // creating a child Span of an ended Transaction. So to avoid the
      //   // code path below that leads to `trans.createSpan()`, we switch to
      //   // using the parent's SpanContext so that a new Transaction is
      //   // created instead.
      //   parentOTelSpanContext = new OTelSpan(parentGenericSpan).spanContext()
      //   parentGenericSpan = null
      }
    } else {
