...
class OTelSpan {
  ...
  _spanCompatMapping () {
    ...
    if (attrs['db.system']) {
      ...
      resource = netName || subtype
      if (attrs['messaging.destination']) {
        resource += '/' + attrs['messaging.destination']
      }
    } else if (attrs['rpc.system']) {
      type = 'external'
      subtype = attrs['rpc.system']
      resource = netName || subtype
      if (attrs['rpc.service']) {
        resource += '/' + attrs['rpc.service']
      }
      ...
      if (httpHost) {
        ...
        resource = parseNetName(attrs['http.url'])
      }
    }
    ...
    if (resource) {
      // XXX I'm not sure if I should be setting `context.destination.{address,port}`
      //   on these spans, if I have that info... or if APM Server handles that.
      this._span.setDestinationContext({
        service: {
          resource,
          // Empty strings per https://github.com/elastic/apm/blob/main/specs/agents/tracing-spans-destination.md#contextdestinationservicenamecontextdestinationservicetype
          name: '',
          type: ''
        }
      })
