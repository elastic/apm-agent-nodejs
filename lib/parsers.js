...
exports.parseError = function (err, options, callback) {
  stackman(err, function (stack) {
    ...
    setCulprit(options, stack.frames)
    setModule(options)
...
function setCulprit(options, frames) {
  if (options.culprit) return // skip if user provided a custom culprit
...
function setExtraProperties(options, properties) {
  ...
  if (typeof options.extra !== 'object') options.extra = { value: String(options.extra) }
