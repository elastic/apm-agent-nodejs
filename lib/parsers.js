...
function getContextFromRequest (req, conf, type) {
  const captureBody = conf.captureBody === type || conf.captureBody === 'all'
  ...
  if (conf.captureHeaders) {
    ...
    if (conf.filterHttpHeaders && context.headers.cookie) {
      ...
      context.cookies = redactKeysFromObject(context.cookies, conf.sanitizeFieldNamesRegExp)
  ...
  const chunked = typeof transferEncoding === 'string' && transferEncoding.toLowerCase() === 'chunked'
  let body = req.json || req.body || req.payload
  const haveBody = body && (chunked || contentLength > 0)
