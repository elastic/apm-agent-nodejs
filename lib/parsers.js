...
function getContextFromRequest (req, conf, type) {
  ...
  if (conf.captureHeaders) {
    context.headers = removeKeysFromObject(
  ...
  if (haveBody) {
    if (captureBody) {
      body = removeKeysFromPostedFormVariables(body, req.headers, conf.sanitizeFieldNamesRegExp)
...
function getContextFromResponse (res, conf, isError) {
  ...
  if (conf.captureHeaders) {
    ...
    context.headers = removeKeysFromObject(context.headers, conf.sanitizeFieldNamesRegExp)
  }
