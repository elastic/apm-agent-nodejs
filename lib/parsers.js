...
function parseUrl (urlOrReq) {
  if (typeof urlOrReq === 'string') {
    return new url.URL(urlOrReq, 'relative:///')
  }

  if (urlOrReq && urlOrReq.url) {
    return parseUrlFromReq(urlOrReq)
  }

  return undefined
}

// same behavior like `parseurl` package
function parseUrlFromReq (req) {
  if (!req.url) return

  if (req._parsedUrl && req._parsedUrl._raw === req.url) {
    return req._parsedUrl
  }

  const parsed = new url.URL(req.url, 'relative:///')

  parsed._raw = req.url

  // Special case for a simple path URL
  const simplePathPattern = /^(\/\/?(?!\/)[^?\s]*)(\?[^\s]*)?$/
  const simplePath = simplePathPattern.exec(req.url)

  if (simplePath && !parsed.url) {
    parsed.pathname = simplePath[1]
  }
