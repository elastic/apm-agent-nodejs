...
function createApmClient (config, agent) {
  ...
  if (config.disableSend || config.contextPropagationOnly) {
    client = new NoopApmClient()
  } else if (typeof config.transport !== 'function') {
    client = new ElasticAPMHttpClient(getHttpClientConfig(config, agent))
  }

  // All clients should report same events: config, error, request-error
  if (client) {
