...
function Client(opts) {
  ...
  if (isLambdaExecutionEnvironment && this._log.isLevelEnabled('trace')) {
    process.prependListener('beforeExit', () => {
      // Avoid using `this._log` because it can result in more async
      // work, leading to an infinite loop of 'beforeExit' events.
      process._rawDebug('trace: elastic-apm-node: process "beforeExit"');
    });
  }
