...
const isOTelMetricsFeatSupported = semver.satisfies(process.version, _supportRange)

function createOTelMetricReader (agent) {
  const metricsInterval = agent._conf.metricsInterval
  assert(metricsInterval > 0, 'createOTelMeterProvider() should not be called if metricsInterval <= 0')
