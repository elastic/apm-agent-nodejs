...
class ElasticApmMetricExporter {
  ...
  export (resourceMetrics, resultCallback) {
    ...
    for (const scopeMetrics of resourceMetrics.scopeMetrics) {
      ...
      for (const metricData of scopeMetrics.metrics) {
        ...
        // `disableMetrics` config handling
        if (this._agent._isMetricNameDisabled(metricName)) {
          continue
        }
        if (!(metricData.dataPointType === DataPointType.GAUGE ||
          ...
          if (!this._dataPointTypeDropWarnCache.has(metricName)) {
            this._agent.logger.warn(`dropping metric "${metricName}": cannot export metrics with dataPointType=${metricData.dataPointType}`)
            this._dataPointTypeDropWarnCache.set(metricName, true)
          }
        }

        for (const dataPoint of metricData.dataPoints) {
          const labels = this._labelsFromOTelMetricAttributes(dataPoint.attributes, metricData.descriptor.name)
   ...
   * Elastic APM intake.
   *
   * This may make in-place changes to the given `attrs` argument. It returns
   * the same object.
   *
   * XXX spec link when merged
   * Spec: https://github.com/elastic/apm/pull/742/files#diff-a04e98daf311e4b4d6a186717a32577382b938c32ebcfc3a73f3b322e584532eR133
   *
   * - XXX APM server >=v7.11 sanitizes '.', '*', and '"' to underscore in
   *   label names. Still under spec discussion if the agent needs to do this
   *   for earlier versions.
   * - XXX Still under spec discussion if adding `otel.scope.{name,version}`
   * - Drop array-valued attrs, and warn (once) with the metric name.
   */
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (Array.isArray(v)) {
        ...
        if (!this._attrDropWarnCache.has(cacheKey)) {
          this._agent.logger.warn({ metricName, attrName: k },
