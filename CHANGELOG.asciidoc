...
* Change how `@hapi/hapi` instrumentation includes additional data when
  ...
  and high memory usage. ({issues}4503[#4503])
+
...
types (boolean, string, number, Date). This is similar behavior as is used for
the `captureAttributes` option to <<apm-capture-error,`apm.captureError()`>>
