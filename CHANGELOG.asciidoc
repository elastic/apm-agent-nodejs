...
* Improve data flushing between the APM agent and the Elastic AWS Lambda
  Extension. This fixes cases where some APM events could be dropped, and
  should improve Lambda function latency with the explicit end signaling from
  agent to extension. However, because this change triggers a bug in the
  extension, this version of the APM Node.js Agent must only be used with
  versions of the
