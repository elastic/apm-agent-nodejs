...
reference to the APM Transaction (and any Spans) until the async task completes.
Before this change the APM Transaction would unintentionally prevent HTTP
request and response objects from being garbage collected -- leading to high
memory usage.
+
