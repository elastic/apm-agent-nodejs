...
- Agent will delay loading of the the `error-callsites` module until agent start time,
  and will not load the module if the agent is disabled/un-active. This prevents the
  setting of an Error.prepareCallStack handler and any other side-effects from 
  `error-callsites` ({issues}2833[#2833] {pull}2906[#2906])
