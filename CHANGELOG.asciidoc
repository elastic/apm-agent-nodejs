...
* Improve error handling with AWS Lambda. When used together with the
  ...
  with a Lambda timeout, `uncaughtException`, or `unhandledRejection` then
  the Lambda extension will report the failed transaction so it can be
  seen in the Kibana APM app. ({issues}3136[#3136])
