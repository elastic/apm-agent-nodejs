...
|https://www.npmjs.com/package/mongoose[mongoose] |>=4.0.0 <5.7.0 |Supported via mongodb-core. Instrumentation is not supported when using the deprecated <<context-manager,`contextManager=patch`>> configuration option
|https://www.npmjs.com/package/mongoose[mongoose] |>=5.7.0 <8 |Supported via mongodb. Instrumentation is not supported when using the deprecated <<context-manager,`contextManager=patch`>> configuration option
