...
To explicitly start the agent you need to import `elastic-apm-node` and call `apm.start()` method with the <<agent-configuration-object, agent configuration object>>.
