...
This configuration option provides a way to override the APM agent's default technique for tracking Node.js async tasks, sometimes referred to as "run context" or just context. Most users should not need to use this setting.
...
[small]#Deprecated in: v3.37.0#

* *Type:* Boolean
* *Env:* `ELASTIC_APM_ASYNC_HOOKS`

This option is *deprecated* -- use <<context-manager,`contextManager`>> instead. The equivalent to `asyncHooks: false` is `contextManager: 'patch'`.
