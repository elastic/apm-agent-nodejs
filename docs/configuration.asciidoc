...
incompatible logger APIs, a given a custom logger is wrapped in a logger that
drops fields other than the log _message_. As a special case, if the provided
logger is a _pino logger instance_, then it will be used directly. Note, as
well, that as of version 3.13 the <<log-level,`logLevel`>> configuration
value *applies* to a custom logger.
