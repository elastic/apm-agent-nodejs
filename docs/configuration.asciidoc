...
Set the verbosity level for the agent. Note that this does not have any
influence on the types of errors that are sent to the APM Server. This only
...
Pass in a custom logger. The custom logger must provide `trace`, `debug`,
`info`, `warn`, `error`, and `fatal` methods that take a string message
argument.

As of version 3.13, the agent uses structured logging using the
https://getpino.io/#/docs/api?id=logger[pino API]. To avoid issues with
incompatible logger APIs, a given a custom logger is wrapped in such a way
that only the log _message_ is passed through -- structured log data is
dropped. As a special case, if the provided logger is a
_pino logger instance_, then it will be used directly without loss of structured
fields.
...
used in agent versions before 3.13, use the following:
...
If no custom logger is provided, the agent will log to stdout in
https://www.elastic.co/guide/en/ecs-logging/overview/current/intro.html[ecs-logging]
format.
