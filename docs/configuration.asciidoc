...
Used to restrict requests for certain URLs from being instrumented.

This property should be set to an array containing one or more string patterns.

When an incoming HTTP request is detected, its URL will be tested against each element 
in this list.  The `transactionIgnoreUrls` property supports exact string matches, and
simple wildcard (`*`) matches.  You may turn on case sensitivity by using the 
`(?+i)` prefix.
