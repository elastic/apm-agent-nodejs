...
③ Finally, you can use the OpenTelemetry Metrics API, to [create metrics](https://open-telemetry.github.io/opentelemetry-js/interfaces/_opentelemetry_api.Meter.md) and the APM agent will periodically ship those metrics to your Elastic APM deployment where you can visualize them in Kibana.
...
* Span events ([`Span#addEvent()`](https://open-telemetry.github.io/opentelemetry-js/interfaces/_opentelemetry_api.Span.md#addEvent)) are not currently supported. Events will be silently dropped.
* [Propagating baggage](https://open-telemetry.github.io/opentelemetry-js/classes/_opentelemetry_api.PropagationAPI.md) within or outside the process is not supported. Baggage items are silently dropped.
