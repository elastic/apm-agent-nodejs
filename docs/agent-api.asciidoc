...
see the <<setup,Setup and Configuration>> guide.
...
Starts the Elastic APM agent for Node.js and returns it self.

IMPORTANT: Put the call to this function at the very top of your main app file - before requiring/importing any other modules.
...
The The secret token optionally expected by the APM Server.
...
or any other string that might help you pinpoint a specific version or deploy.
...
An error will be generated and sent to the APM Server in if this happens.
...
Set the verbosity level the agent.
Note that this does not have any influence on what types of errors that are sent to the APM Server.
This only controls how chatty the agent are in your logs.
...
Whether or not the agent should monitor for uncaught exceptions and sent them to the APM Server automatically.
...
Set it option to `false` to disable capture of stack traces for measured traces during instrumentation.
...
The agent maintains an in-memory queue to which recorded metrics and sampled transactions are added when they end.
By default,
...
NOTE: The queue is flushed only 5 seconds after the first transaction have ended on a newly started Node process.
...
Each time the properties of the `context` argument is shallow merged with the context previously given.

If an error is captured,
the context from the active transaction is used as context for the captured error and any custom context given as the 2nd argument to <<apm-capture-error,`apm.captureError`>> takes precedence and is shallow merged on top.
...
The optional `callback` will be called once the error have been sent to the APM Server.
...
To ease debugging it's possible to send some extra data with each error you sent to the APM Server.
...
For more details on the properties accepted by the error intake API see the https://www.elastic.co/guide/en/apm/server/current/_error_api.html[intake error API docs].
...
Besides the options described in the <<metadata,the metadata section>>,
...
The agent provided middleware can be used as-is with either Connect or Express in the same way.
Take note that in your middlewares,
the agent middleware must appear _after_ your router to pick up any errors that may result from handling a request.
...
NOTE: `apm.middleware.express` or `apm.middleware.connect` _must_ be added to the middleware stack _before_ any other error handling middlewares or there's a chance that the error will never get to the agent.
...
See <<transaction-api,Transaction API>> docs for details on how to use custom transactions.
...
the node process is terminated automatically when an uncaught exception have been captured and sent to the APM Server.
