[[custom-stack]]
== Getting started with a custom Node.js stack
...
and other slow operations that happens during requests to your app.
...
Example application using the https://github.com/watson/patterns[patterns] module for route handling:
...
which by definition doesn't match any route,
...
including adding extra meta-data to the error,
...
But in many cases,
information about the HTTP request it self isn't enough.
