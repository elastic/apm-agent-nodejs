...
You may run into a few subtle surprises when starting the Elastic Node.js APM agent with certain technologies. A general troubleshooting tip for using the agent with any build tool/system that produces compiled JavaScript is to look at the compiled JavaScript to see what is actually being executed by `node`.
