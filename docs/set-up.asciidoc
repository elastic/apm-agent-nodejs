...
All imports are "hoisted" to the top of a module, properly following ECMAScript module (ESM) semantics. However, the `apm.start()` is now too late: *after* the `http` module has been imported. This is why <<start-option-2,the `elastic-apm-node/start` module>> was added as a start option. The following will work:
