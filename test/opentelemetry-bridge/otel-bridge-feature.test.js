...
tape.test('APM Server <v8.0 (which requires unsampled transactions)', function (suite) {
  ...
  suite.test('Scenario Outline: OTel span kind <kind> for spans & default span type & subtype', function (t) {
    const examples = [
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        ...
        t.equal(span.subtype, ex.subtype, 'Then Elastic bridged span subtype is "<default_subtype>"')
      }
  ...
  suite.test('Scenario Outline: OTel span kind <kind> for transactions & default transaction type', function (t) {
    const examples = [
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        ...
        t.equal(trans.otel.span_kind, otel.SpanKind[ex.kind], 'Then Elastic bridged transaction OTel kind is "<kind>"')
        t.equal(trans.type, 'unknown', 'Then Elastic bridged transaction type is "unknown"')
      }
  ...
  suite.test('Scenario Outline:  OTel span mapping with status <status> for transactions', function (t) {
    const examples = [
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        ...
        const trans = transactions[i]
        t.ok(trans, 'Then Elastic bridged object is a transaction')
        t.equal(trans.outcome, ex.outcome, 'Then Elastic bridged transaction outcome is "<outcome>"')
      }
  ...
  suite.test('Scenario Outline:  OTel span mapping with status <status> for spans', function (t) {
    const examples = [
      { status: otel.SpanStatusCode.UNSET, outcome: OUTCOME_UNKNOWN },
      { status: otel.SpanStatusCode.OK, outcome: OUTCOME_SUCCESS },
      { status: otel.SpanStatusCode.ERROR, outcome: OUTCOME_FAILURE }
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        ...
        const span = spans[i]
        t.ok(span, 'Then Elastic bridged object is a span')
        t.equal(span.outcome, ex.outcome, 'Then Elastic bridged span outcome is "<outcome>"')
      }
  ...
  suite.test('Scenario Outline: HTTP server [ <http.url> <http.scheme> ]', function (t) {
    const examples = [
      { attributes: { 'http.url': 'http://testing.invalid/' } },
      { attributes: { 'http.scheme': 'http' } }
    ]
    examples.forEach((ex, idx) => {
      tracer.startSpan(`s${idx}`, { kind: otel.SpanKind.SERVER, attributes: ex.attributes }).end()
    })
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        const trans = transactions[i]
        t.ok(trans, 'Then Elastic bridged object is a transaction')
        t.equal(trans.type, 'request', 'Then Elastic bridged transaction type is "request"')
      }
  ...
  suite.test('Scenario Outline: HTTP client [ <http.url> <http.scheme> <http.host> <net.peer.ip> <net.peer.name> <net.peer.port> ]', function (t) {
    const examples = [
      {
    ...
    tracer.startActiveSpan('aTrans', aTrans => {
      examples.forEach((ex, idx) => {
        tracer.startSpan(`s${idx}`, { kind: otel.SpanKind.CLIENT, attributes: ex.attributes }).end()
      })
      aTrans.end()
    })
  ...
  suite.test('Scenario Outline: DB client [ <db.system> <net.peer.ip> <net.peer.name> <net.peer.port>]', function (t) {
    const examples = [
      {
  ...
  suite.test('Scenario: Messaging consumer', function (t) {
    ...
    agent.flush(function () {
      ...
      t.equal(trans.type, 'messaging', 'Then Elastic bridged transaction type is "messaging"')
  ...
  suite.test('Scenario Outline: Messaging producer [ <messaging.system> <messaging.destination> <messaging.url> <net.peer.ip> <net.peer.name> <net.peer.port>]', function (t) {
    const examples = [
      {
        attributes: {
    ...
    tracer.startActiveSpan('aTrans', aTrans => {
      examples.forEach((ex, idx) => {
        tracer.startSpan(`s${idx}`, { kind: otel.SpanKind.PRODUCER, attributes: ex.attributes }).end()
      })
      aTrans.end()
    })
    ...
    agent.flush(function () {
      ...
      for (let i = 0; i < examples.length; i++) {
        const ex = examples[i]
        const span = spans[i]
        t.equal(span.type, 'messaging', 'Then Elastic bridged span type is "messaging"')
  ...
  suite.test('Scenario Outline: RPC client [ <rpc.system>  <rpc.service> <net.peer.ip> <net.peer.name> <net.peer.port>]', function (t) {
    const examples = [
      {
        attributes: {
          'rpc.system': 'grpc'
