...
tape.test('OTel context attachment works', t => {
  ...
  otel.context.with(ctx, function () {
    t.strictEqual(otel.context.active(), ctx, 'active context is the same Context instance as was activated')
...
tape.test('OTel span in an Elastic span', t => {
  ...
  t.strictEqual(parentIdFromOTelSpan(s3), t3.id,
...
tape.test('Elastic span in an OTel span', t => {
  tracer.startActiveSpan('t4', t4 => {
    ...
    t.strictEqual(s4.parentId, idFromOTelSpan(t4), 'Elastic span created in context of OTel span is a child of it')
    s4.end()
    t4.end()
    t.end()
  })
