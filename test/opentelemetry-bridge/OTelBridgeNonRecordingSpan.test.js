...
const { OTelBridgeNonRecordingSpan } = require('../../lib/opentelemetry-bridge/OTelBridgeNonRecordingSpan')
...
tape.test('OTelBridgeNonRecordingSpan', suite => {
  ...
  suite.test('OTel interface Span', t => {
    ...
    t.equal(nrsOTelSpan.setAttribute('foo', 'bar'), nrsOTelSpan, 'setAttribute')
    t.equal(nrsOTelSpan.setAttributes({ foo: 'bar' }), nrsOTelSpan, 'setAttributes')
    t.equal(nrsOTelSpan.addEvent('anEvent'), nrsOTelSpan, 'addEvent')
    t.equal(nrsOTelSpan.setStatus(otel.SpanStatusCode.OK), nrsOTelSpan, 'setStatus')
    ...
    t.equal(nrsOTelSpan.recordException(new Error('boom')), undefined, 'recordException')
    t.end()
  })
  ...
  suite.test('public interface Transaction API', t => {
    ...
    t.deepEqual(nrsTrans.ids,
      { 'trace.id': 'd4cda95b652f4a1592b449dd92ffda3b', 'transaction.id': '6e0c63ffe4e34c42' },
      'ids')

    t.equal(nrsTrans.setType('aType', 'aSubtype', 'anAction'), undefined, 'setType')
