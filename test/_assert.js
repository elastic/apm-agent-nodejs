...
exports.stacktrace = function (t, topFunctionName, topAbsPath, stacktrace, _agent, isError) {
  ...
  t.strictEqual(stacktrace[0].function, topFunctionName, 'top frame should have expected function')
  t.strictEqual(stacktrace[0].abs_path, topAbsPath, 'top frame should have expected abs_path')
...
function stackFrameValidator (t, agent, isError) {
  ...
  return function (frame) {
    ...
    t.strictEqual(typeof frame.filename, 'string', 'frame.filename should be a string')
    t.ok(frame.lineno > 0, 'frame.lineno should be greater than 0')
    t.strictEqual(typeof frame.function, 'string', 'frame.function should be a string')
    t.strictEqual(typeof frame.library_frame, 'boolean', 'frame.library_frame should be a boolean')
    t.strictEqual(typeof frame.abs_path, 'string', 'frame.abs_path should be a string')

    if (shouldHaveSource) {
      t.ok(Array.isArray(frame.pre_context), 'frame.pre_context should be an array')
      t.strictEqual(frame.pre_context.length, 2, 'frame.pre_context should have two elements')
      t.strictEqual(typeof frame.context_line, 'string', 'frame.context_line should be a string')
      ...
      t.strictEqual(frame.post_context.length, 2, 'frame.post_context should have two elements')
    }
  }
}
