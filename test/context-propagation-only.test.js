...
tape.test('contextPropagationOnly', function (suite) {
    ...
    server.start(function (serverUrl_) {
      ...
      apm.start({
        ...
        transport: function customTransport (conf, agent) {
          throw new Error('custom transport should not get called with contextPropagationOnly=true')
        },
  ...
  suite.test('transport should be NoopTransport if contextPropagationOnly=true', function (t) {
  ...
  suite.test('ensure distributed tracing and log enrichment still works', function (t) {
    let headersA
    let traceparentA
    let traceparentB
    ...
    serviceA.listen(function () {
      ...
      serviceB.listen(function () {
        ...
        t.equal(apm.currentTraceparent, null,
