...
const testFixtures = [
  {
    name: 'cassandra-driver simple usage (<4.8.0)',
  ...
  },
  {
    ...
    versionRanges: {
      // - cassandra-driver >=4.8.0 only supports node >=18
      node: '>=18',
      'cassandra-driver': '>=4.8.0',
    },
    // verbose: true,
  ...
  {
    name: 'cassandra-driver ESM (<4.8.0)',
...
function checkApmServerEsm(t, apmServer) {
  t.ok(apmServer.events[0].metadata, 'metadata');
  const events = sortApmEvents(apmServer.events);
