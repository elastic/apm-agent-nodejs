...
function checkExpectedApmEvents (t, apmEvents) {
  ...
  if (apmEvents.length > 0) {
    ...
    t.equal(metadata.cloud.instance.name, 'AJsAzureFnApp', 'metadata.cloud.instance.name')
    t.equal(metadata.cloud.project.name, 'my-resource-group', 'metadata.cloud.project.name')
