...
testData.forEach(testDatum => {
  test(testDatum.failure_message, t => {
    ...
    const span = apm.startSpan('aSpan', testDatum.span.type, testDatum.span.subtype, spanOpts);
    if (testDatum.span.context) {
      ...
      if (testDatum.span.context.http) {
        ...
        if (httpContext.url) {
          ...
          if (httpContext.url.host) { u.hostname = httpContext.url.host; }
          if (httpContext.url.port) { u.port = httpContext.url.port; }
      ...
      if (testDatum.span.context.service && testDatum.span.context.service.target) {
        span.setServiceTarget(testDatum.span.context.service.target.type, testDatum.span.context.service.target.name);
      }
    }
    span.end();
    ...
    apm.flush(() => {
      ...
      if (testDatum.expected_resource === null) {
        ...
        t.equal(spanPayload.context.destination.service.resource,
          testDatum.expected_resource,
          'span.context.destination.service.resource');
      }
      if (testDatum.expected_service_target === null) {
        t.ok(!(spanPayload.context && spanPayload.context.service && spanPayload.context.service.target),
          'no span.context.service.target');
      } else {
        t.deepEqual(spanPayload.context.service.target,
