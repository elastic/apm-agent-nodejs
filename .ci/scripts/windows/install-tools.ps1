...
& choco list nodejs --exact --by-id-only --all | Select-String -Pattern "nodejs $env:VERSION"
$Version = $(choco list nodejs --exact --by-id-only --all) | Select-String -Pattern "nodejs $env:VERSION" | %{$_.ToString().split(" ")[1]} | sort {[version] $_} | Select-Object -Last 1
