...
# Run the microbenchmark in GitHub actions and for such
# it configures the required settings in the GitHub hosted runners
# to execute the benchmarks afterwards

## GitHub action specific configuration
if [ "${CI}" == "true" ] ; then
  # If HOME is not set then use the GitHub workspace
